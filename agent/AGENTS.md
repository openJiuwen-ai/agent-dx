...
- Gateway 装配 Agent 入口，默认内嵌 Activator，通过 LocalControl 与本地 Sandbox 方法调用；可显式配置 remote 使用独立 Activator。内嵌模式由 Activator 访问 ADX Redis，业务层仍通过 Control/Sandbox 边界；独立模式 Gateway 不连接 ADX Redis。按真实 Sandbox ID 转发，不添加 Template/Target 本地缓存或旁路参数。inline 管理独立适配旧协议；HTTP/WS/SSH 与 v2 共享访问入口，用 instance/target 选择内部认证与协议分支，旧凭据不能回退到 v2 的 Platform API Key 验证；create/get/kill 独立于 Activator 和 ADX 持久化。
