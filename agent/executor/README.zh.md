...
Executor 不依赖 agent-dx SDK；当前子 Sandbox 管理仍使用外部 `yr` SDK，
镜像必须包含兼容的 Python Runtime。改接 Sandbox SDK 属于后端功能迁移。

Executor 需安装到宿主 Runtime 使用的 Python 环境中。它属于镜像基础设施，不占用
...
系统上，使 `adx.agentexecutor` 与包含原生库的宿主 Runtime 位于同一目录。
...
Agent 进程通过 loopback HTTP 调用。当前保留导入的执行行为，子 Sandbox 后端仍依赖外部运行时 SDK：
