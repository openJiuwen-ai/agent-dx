...
if TYPE_CHECKING:
    from adx.agentexecutor.sandbox.sandbox import Sandbox
...
def _get_gateway_host() -> str:
    ...
    Resolution order:
    1. ``ADX_GATEWAY_ADDRESS`` environment variable
    2. ``ADX_SERVER_ADDRESS`` environment variable
    3. :class:`~yr.config_manager.ConfigManager` ``server_address``
    """
    from yr.config_manager import ConfigManager

    host = os.environ.get("ADX_GATEWAY_ADDRESS", "").strip()
    if host:
        return host
    addr = os.environ.get("ADX_SERVER_ADDRESS", "").strip()
    if addr:
        return addr
...
class SandboxFilesystem:
    ...
    def copy_from_local(
        ...
        """
        self._cp(local_path, remote_path, CpDirection.UPLOAD, streaming, trace_id=trace_id)
    ...
    def copy_to_local(
        ...
        Raises:
            RuntimeError: Server address is not configured.
        """
        self._cp(remote_path, local_path, CpDirection.DOWNLOAD, streaming, trace_id=trace_id)
    ...
    def _resolve_transport_kwargs(self, host: str, port: str) -> dict:
        ...
        * **use_ssl** — ``True`` if any of: port is ``443``,
          ...
          CA file configured, or env ``ADX_USE_SSL`` is truthy.
        ...
        * **verify_server** — ``True`` by default; ``False`` if env
          ``ADX_INSECURE`` is truthy or ``ADX_VERIFY_SERVER`` is falsey.
        * **token** — ``ConfigManager.auth_token`` or env ``ADX_AUTH_TOKEN``.
        """
        from yr.config_manager import ConfigManager
        ...
        env_ssl = self._env_bool("ADX_USE_SSL")
        if env_ssl is not None:
            use_ssl = env_ssl

        insecure = self._env_bool("ADX_INSECURE", False)
        verify_server = self._env_bool("ADX_VERIFY_SERVER", True)
        if insecure:
            verify_server = False
    ...
    def _get_connection(self):
        ...
        if not gateway_addr:
            raise RuntimeError(
                ...
                "Set ADX_SERVER_ADDRESS (faasfrontend address) or call yr.init()."
            )
