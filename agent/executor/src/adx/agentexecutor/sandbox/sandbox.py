...
from adx.agentexecutor.sandbox.filesystem import CpDirection, SandboxFilesystem, _get_gateway_host
...
def _sandbox_trace_enter(op: str, trace_id: Optional[str], request_id: str,
    ...
    if trace_id:
        _validate_trace_id(trace_id)
    fields = [f"adx.trace_id={trace_id}" if trace_id else None,
...
def _print_gateway_urls(instance_id: str, port_forwardings: List["PortForwarding"]) -> None:
    ...
    if not gateway_host:
        logger.warning("cannot print port forwarding URLs: ADX_GATEWAY_ADDRESS or ADX_SERVER_ADDRESS not set")
        return
...
class SandboxInstance:
    ...
    def __init__(
        ...
        if working_dir is None:
            self._temp_dir = tempfile.TemporaryDirectory(prefix="adx_sandbox_")
        ...
        self.__adx_before_snapshot__ = None
        self.__adx_after_snapstart__ = None
    ...
    def get_internal_urls() -> Dict[int, str]:
        ...
        """
        host_ip = os.environ.get("ADX_INTERNAL_HOST_IP", "")
        pf_str = os.environ.get("ADX_PORT_FORWARDINGS", "")
    ...
    def start_tunnel_server(ws_port: int = 8765, http_port: int = 8766) -> None:
        ...
        from adx.agentexecutor.sandbox.tunnel_server import TunnelServer
    ...
    def read_file(path: str, mode: str = "rb"):
        ...
        Examples:
            ...
            >>> print(content)
        """
        from adx.agentexecutor.sandbox.filesystem import _read_file_impl
    ...
    def write_file(path: str, data, mode: str = "wb") -> None:
        ...
        """
        from adx.agentexecutor.sandbox.filesystem import _write_file_impl
    ...
    def list_files(
        ...
        """
        from adx.agentexecutor.sandbox.filesystem import _list_files_impl
    ...
    def search_files(
        ...
        Raises:
            ...
            OSError: If the path cannot be accessed.
        """
        from adx.agentexecutor.sandbox.filesystem import _search_files_impl
    ...
    def execute(
        ...
        Examples:
            >>> sandbox = adx.agentexecutor.sandbox.Sandbox.invoke()
...
def create(
    *args: str,
    ...
    Examples:
        ...
        >>> yr.init()
        >>>
        >>> sandbox = adx.agentexecutor.sandbox.create()
        >>> # Create sandbox with supervisor executor
        >>> sandbox = adx.agentexecutor.sandbox.create(sandbox_type="supervisor")
        >>> result = yr.get(sandbox.exec("pwd"))
        >>> print(result['stdout'])
        >>>
        >>> # Create sandbox with docker executor
        >>> sandbox = adx.agentexecutor.sandbox.create(sandbox_type="docker", image="python:3.12-slim")
...
def restore(
    ...
    Examples:
        ...
        >>> restored = adx.agentexecutor.sandbox.restore(checkpoint_id)
...
class Sandbox:
    ...
    Examples:
        ...
        >>>
        >>> # Basic sandbox
        >>> sb = adx.agentexecutor.sandbox.Sandbox()
        >>> result = yr.get(sb.exec("echo hello"))
        >>> print(result['stdout'])
        >>>
        >>> # Sandbox with reverse tunnel to local service
        >>> sb = adx.agentexecutor.sandbox.create(upstream="upstream.example.com:8000")
    ...
    def create_new_instance(self, options: SandboxCreateOptions):
        ...
        if ports is not None:
            ...
            for port_forward in ports:
                ...
                if len(parts) == 2:
                    ...
                    adx_port_forwardings.append(PortForwarding(port=port, protocol=protocol.upper()))
                elif len(parts) == 1:
                    ...
                    adx_port_forwardings.append(PortForwarding(port=port, protocol="TCP"))
                else:
                    raise ValueError(
                        f"Invalid port_forwarding format: {port_forward}. "
                        "Expected format: protocol:port (e.g., tcp:8080)"
                    )
            opt.port_forwardings = adx_port_forwardings
        ...
        if upstream is not None:
            ...
            from adx.agentexecutor.sandbox.tunnel_client import TunnelClient
    ...
    def get_tunnel(self, port: int) -> str:
        ...
        traefik_address = os.environ.get("ADX_GATEWAY_ADDRESS", "")
        if traefik_address == "":
            raise ValueError(f"ADX_GATEWAY_ADDRESS is not set.")
    ...
    def checkpoint(
        ...
        can be used with ``adx.agentexecutor.sandbox.restore()`` or ``Sandbox(checkpoint_id=...)`` to
        ...
        Examples:
            >>> sandbox = adx.agentexecutor.sandbox.create(rootfs="python:3.12-slim")
    ...
    def cp(
        ...
        * ``CpDirection.DOWNLOAD`` – ``src`` is a path **inside the sandbox**,
        ...
        Examples:
            >>> sb = adx.agentexecutor.sandbox.create()
            >>> # Upload a local file to the sandbox (default direction)
            >>> sb.cp("/local/data.csv", "/sandbox/data.csv")
            >>> # Download a file from the sandbox
            >>> from adx.agentexecutor.sandbox.filesystem import CpDirection
...
def main():
    ...
    os.environ.pop("ADX_WORKING_DIR", None)
