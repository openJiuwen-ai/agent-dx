...
{
  "runtime": "python3.11",
  "handler": "adx.agentruntime.bootstrap.handler",
  "extendedHandler": {
    "initializer": "adx.agentruntime.bootstrap.initialize"
  },
  "extendedTimeout": {
    "initializer": 60
  },
...
`adx/agentruntime/bootstrap.py` 包含在函数代码包中。

调度器必须开启 `enableSessionCtx`，才能绑定并注入 `ADX_SESSION_CTX_ID`。本 SDK
...
`ADX_DATASYSTEM_DEFAULT_WRITE_MODE` 配置，默认值为 `NONE_L2_CACHE`。
