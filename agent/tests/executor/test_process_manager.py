...
from adx.agentexecutor.process_manager import MAX_BOOTSTRAP_COMMANDS, ProcessManager, parse_bootstrap_commands
...
def test_process_manager_reads_runtime_id_from_env(tmp_path):
    manager = ProcessManager()
    environ = {
        "ADX_RUNTIME_BOOTSTRAP_CMD": json.dumps([[sys.executable, "-c", "print('env-runtime-log')"]]),
        "GLOG_log_dir": str(tmp_path),
        "ADX_RUNTIME_ID": "runtime-from-env",
    }
...
def test_process_manager_logs_trace_and_instance_ids(tmp_path, caplog):
    ...
    manager = ProcessManager()
    environ = {
        "ADX_RUNTIME_BOOTSTRAP_CMD": json.dumps([[sys.executable, "-c", "print('traced')"]]),
        "GLOG_log_dir": str(tmp_path),
        "ADX_RUNTIME_ID": "rt-1",
        "ADX_TRACE_ID": "t-001",
