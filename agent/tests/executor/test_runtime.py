...
from adx.agentexecutor import handler
from adx.agentexecutor.probe import ProbeSpec, ProbeStartupError
from adx.agentexecutor.runtime import AgentExecutorRuntime, resolve_process_shutdown_grace
...
def test_shutdown_grace_honors_override_without_exceeding_budget():
    assert resolve_process_shutdown_grace({
        "PRE_STOP_TIMEOUT": "10",
        "ADX_AGENT_EXECUTOR_SHUTDOWN_GRACE_SECONDS": "3",
    }) == 3
    assert resolve_process_shutdown_grace({
        "PRE_STOP_TIMEOUT": "10",
        "ADX_AGENT_EXECUTOR_SHUTDOWN_GRACE_SECONDS": "20",
    }) == 8
...
def test_start_raises_on_startup_failure_without_deadlock(monkeypatch):
    ...
    monkeypatch.setattr("adx.agentexecutor.runtime.ProcessManager", lambda: pm)
    ...
    monkeypatch.setattr(
        "adx.agentexecutor.runtime.ExecutorHTTPServer",
    ...
    monkeypatch.setattr("adx.agentexecutor.runtime.run_startup", _raise_startup)
    monkeypatch.setattr("adx.agentexecutor.runtime._load_probe_set", lambda: (startup_spec, None))
    monkeypatch.setenv("PRE_STOP_TIMEOUT", "10")
...
def test_start_logs_agent_trace_startup_line(monkeypatch, caplog):
    ...
    monkeypatch.setenv("ADX_TRACE_ID", "t-004")
    monkeypatch.setenv("INSTANCE_ID", "default-alice+coder")
    monkeypatch.setenv("ADX_RUNTIME_ID", "rt-9")
    ...
    monkeypatch.setattr(
        "adx.agentexecutor.runtime.ProcessManager.start_from_env", lambda self, environ=None: None
    )
    runtime = AgentExecutorRuntime()
    runtime.start()
    try:
