...
from adx.agentruntime import AgentExecutor, Complete, InputRequired
from adx.agentruntime.bootstrap import AgentRuntime, RuntimeState, _load_executor
from adx.agentruntime.errors import (
    AgentExecutorLoadFailed,
    DataSystemError,
    SessionContextBindingMismatch,
)
from adx.agentruntime.storage import MemoryKVStore
...
class BootstrapTest(unittest.TestCase):
    def _initialize(self, agent, store=None, session_id="session"):
        ...
        with patch.dict("os.environ", {"ADX_SESSION_CTX_ID": session_id}, clear=True):
            runtime.initialize(
                context,
    ...
    def test_cached_datasystem_error_is_sanitized(self):
        ...
        with patch.dict("os.environ", {"ADX_SESSION_CTX_ID": "session"}, clear=True):
            runtime.initialize(
                FakeFunctionContext(),
                store=MemoryKVStore(),
    ...
    def test_repeated_initialize_rejects_a_different_session_binding(self):
        ...
        with patch.dict(
            "os.environ", {"ADX_SESSION_CTX_ID": "session-b"}, clear=True
        ):
            runtime.initialize(
                context,
                store=MemoryKVStore(),
