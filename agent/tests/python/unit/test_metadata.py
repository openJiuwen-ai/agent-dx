...
from adx.agentruntime.errors import AgentRuntimeNotConfigured
from adx.agentruntime.metadata import RuntimeMetadata
...
class MetadataTest(unittest.TestCase):
    ...
    def test_rejects_missing_and_empty_session_context(self):
        for environ in ({}, {"ADX_SESSION_CTX_ID": ""}, {"ADX_SESSION_CTX_ID": " "}):
    ...
    def test_rejects_none_or_empty_function_identity(self):
        for method, value in (
            ...
            with self.subTest(method=method):
                ...
                with self.assertRaises(AgentRuntimeNotConfigured):
                    RuntimeMetadata.from_function_context(
                        context, {"ADX_SESSION_CTX_ID": "session"}
                    )
