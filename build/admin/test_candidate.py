...
SPEC = importlib.util.spec_from_file_location("admin_candidate", MODULE_PATH)
...
class CandidateTests(unittest.TestCase):
    def test_candidate_records_and_verifies_both_distributions(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            wheel = root / "adxadmin-0.1.0-py3-none-any.whl"
            with zipfile.ZipFile(wheel, "w") as archive:
                archive.writestr(
                    "adxadmin-0.1.0.dist-info/METADATA",
                    "Name: adxadmin\nVersion: 0.1.0\nRequires-Python: >=3.10\n",
                )
            (root / "adxadmin-0.1.0.tar.gz").write_bytes(b"source")
    ...
    def test_candidate_rejects_incomplete_or_wrong_package(self):
        with tempfile.TemporaryDirectory() as directory:
            with self.assertRaisesRegex(ValueError, "one wheel"):
                candidate.create(Path(directory), "a" * 40)
