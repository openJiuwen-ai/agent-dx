#!/usr/bin/env python3
"""Linux PID-namespace acceptance; needs CAP_SYS_ADMIN and a real EXECD binary."""
import argparse
import json
import os
...
def run(binary):
    ...
    env={**os.environ,'EXECD_HTTP_ONLY':'1','EXECD_HTTP_PORT':str(port)}
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                if process.poll() is not None or time.monotonic()>end:raise RuntimeError('EXECD failed to start')
                time.sleep(.1)
        def execute(command):
