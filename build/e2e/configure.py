...
if node=='node1':
 ...
 if ingress_mode not in ('embedded','standalone'):raise ValueError('invalid E2E Ingress mode')
 add('api','apiserver',{'listen':'127.0.0.1:8888','loopback_http':True,'ingress_mode':ingress_mode,'discovery':{'poll_seconds':1},'ca':str(T/'ca.pem'),'certificate':str(T/'apiserver.pem'),'private_key':str(T/'apiserver.key'),'server_name':'localhost','rpc_timeout_seconds':120,'cache_entries':1000,'auth_cache_ttl_seconds':10})
 ...
 ee['ADX_DATA_PLANE_INGRESS_PORT_HOST_DOMAIN']='example.test'
