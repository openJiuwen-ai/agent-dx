...
if node=='node1':
 ...
 add('master','master',{'listen':'0.0.0.0:17000','advertised_address':'https://127.0.0.1:17000','domains':1,'placement':'spread','rpc_timeout_seconds':120,'tls':tls('master',{'frontend':'frontend','edge':'edge','node:node1':'node','node:node2':'node2'}),'bootstrap_credentials':[{'key_file':str(key),'tenant_id':'e2e','administrator':False,'expires_at_unix_seconds':0},{'key_file':str(other),'tenant_id':'e2e-other','administrator':False,'expires_at_unix_seconds':0}]})
...
add(node,'node-manager',{'node_id':node,'listen':'0.0.0.0:17001','advertised_address':f'{host}:17001','proxy_address':f'{host}:18443','tls':tls('node' if node=='node1' else 'node2',{'master':'master','frontend':'frontend'}),'sandboxd_socket':str(R/'sandboxd.sock'),'proxy_socket':str(P/'proxy/route.sock'),'resource_source':{'kind':'sandboxd','socket':str(P/'resource.sock'),'valid_for_seconds':6},'degradation_journal':str(P/'degraded/results.sqlite'),'metrics_listen':'127.0.0.1:17003','checkpoint_dir':str(P/'checkpoints'),'report_interval_seconds':2,'rpc_timeout_seconds':20,'rrt_port':50090,'rrt_command':['/usr/local/bin/rrt-runtime'],'rrt_env':{}})
if node=='node1':
