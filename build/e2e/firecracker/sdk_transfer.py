...
result={'status':'failed','cases':[], 'interruption_requested':os.getenv('ADX_TRANSFER_INTERRUPT_COORDINATOR')=='1', 'node_interruption_requested':os.getenv('ADX_TRANSFER_INTERRUPT_NODE')=='1'}; sandbox=None; stopped=None; blocked_rpc=None; blocked_execd=None
...
try:
    ...
    stopped=pid(root/source,'adxlet'); os.kill(stopped,signal.SIGSTOP)
    ...
    if result['interruption_requested']:
        ...
        (root/'evidence/mid-recovery-coordinator-restart.json').write_text(json.dumps(result['mid_recovery_restart'],indent=2))
    ...
    if result['node_interruption_requested']:
        ...
        subprocess.run([*blocked_execd,'-D',*execd_rule],check=True);blocked_execd=None
