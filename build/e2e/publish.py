...
BASE='http://127.0.0.1:5000';REPO='/v2/adx-rrt'
...
def publish(archive):
    ...
    with tarfile.open(archive) as tar:
        ...
        if len(manifest)!=1:raise ValueError('RRT archive must contain exactly one image')
        image=manifest[0]
        ...
        for path in image['Layers']:
            ...
            layers.append({'mediaType':'application/vnd.docker.image.rootfs.diff.tar.gzip','size':len(data),'digest':blob(data)})
        body=json.dumps({'schemaVersion':2,'mediaType':'application/vnd.docker.distribution.manifest.v2+json','config':{'mediaType':'application/vnd.docker.container.image.v1+json','size':len(config),'digest':blob(config)},'layers':layers},separators=(',',':')).encode()
        with request(REPO+'/manifests/acceptance','PUT',body,{'Content-Type':'application/vnd.docker.distribution.manifest.v2+json'}) as response:digest=response.headers['Docker-Content-Digest']
