    ...
    def scenarios(self,checks,required=None):
        ...
        if 'command-response-cut' in selected:
            with self.case('command-response-cut', checks) as record:
                ...
                record['subcases']=sdk_subcases_from_output(output)
                for node in self.nodes:self.helper(node,'empty',node)
        if 'command-watch-unavailable' in selected:
            with self.case('command-watch-unavailable', checks) as record:
                output=self.execute('node1','/opt/adx/client/bin/python','-u',
                                    '/opt/adx/e2e/scenarios.py','command-watch-unavailable',timeout=300)
                record['subcases']=sdk_subcases_from_output(output)
                for node in self.nodes:self.helper(node,'empty',node)
        if 'command-watch-query-unavailable' in selected:
            with self.case('command-watch-query-unavailable', checks) as record:
                output=self.execute('node1','/opt/adx/client/bin/python','-u',
                                    '/opt/adx/e2e/scenarios.py','command-watch-query-unavailable',timeout=300)
                record['subcases']=sdk_subcases_from_output(output)
                for node in self.nodes:self.helper(node,'empty',node)
