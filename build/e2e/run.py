...
OPTIONAL_CASES = ('redis-restart','coordinator-restart','coordinator-adxlet-restart','apiserver-restart','ingress-restart','runtime-affinity','idle-active','relay-standalone','runtime-exit','sandboxd-runtime-loss','sqlite-fallback','create-response-cut','create-unknown-query','command-response-cut','command-registry-capacity','upload-response-cut','download-response-cut','schedule-deadline','resource-stale','network-partition','reconcile-crash','mixed-soak')
PROFILES = {
    'l0': ('l0','auth'),
    'standalone': STANDARD,
    'k8s-basic': BASIC,
    ...
    def scenarios(self,checks,required=None):
        ...
        if 'runtime-affinity' in selected:
            with self.case('runtime-affinity', checks) as record:
                ...
                record['subcases']=sdk_subcases_from_output(output)
                for node in self.nodes:self.helper(node,'empty',node)
        if 'mixed-soak' in selected:
            with self.case('mixed-soak', checks) as record:
                self.event('Sustain concurrent command/file traffic and instance churn on both nodes')
                output=self.execute('node1','/opt/adx/client/bin/python','-u',
        ...
        if 'relay-standalone' in selected:
            with self.case('relay-standalone', checks) as record:
                ...
                record['subcases']=sdk_subcases_from_output(output)
                for node in self.nodes:self.helper(node,'empty',node)
        if 'runtime-exit' in selected:
            with self.case('runtime-exit', checks) as record:
                output=self.execute('node1','/opt/adx/client/bin/python','-u',
                                    '/opt/adx/e2e/scenarios.py','runtime-exit',timeout=360)
                record['subcases']=sdk_subcases_from_output(output)
                for node in self.nodes:self.helper(node,'empty',node)
        if 'sqlite-fallback' in selected:
            with self.case('sqlite-fallback', checks) as record:
        ...
        if 'create-response-cut' in selected:
            with self.case('create-response-cut', checks) as record:
                ...
                record['subcases']=sdk_subcases_from_output(output)
                for node in self.nodes:self.helper(node,'empty',node)
        if 'create-unknown-query' in selected:
            with self.case('create-unknown-query', checks) as record:
                output=self.execute('node1','/opt/adx/client/bin/python','-u',
                                    '/opt/adx/e2e/scenarios.py','create-unknown-query',timeout=300)
                record['subcases']=sdk_subcases_from_output(output)
                for node in self.nodes:self.helper(node,'empty',node)
        if 'command-response-cut' in selected:
            with self.case('command-response-cut', checks) as record:
                output=self.execute('node1','/opt/adx/client/bin/python','-u',
                                    '/opt/adx/e2e/scenarios.py','command-response-cut',timeout=300)
                record['subcases']=sdk_subcases_from_output(output)
                for node in self.nodes:self.helper(node,'empty',node)
        ...
        if 'upload-response-cut' in selected:
            with self.case('upload-response-cut', checks) as record:
                output=self.execute('node1','/opt/adx/client/bin/python','-u',
                                    '/opt/adx/e2e/scenarios.py','upload-response-cut',timeout=300)
                record['subcases']=sdk_subcases_from_output(output)
                for node in self.nodes:self.helper(node,'empty',node)
        if 'download-response-cut' in selected:
            with self.case('download-response-cut', checks) as record:
                output=self.execute('node1','/opt/adx/client/bin/python','-u',
                                    '/opt/adx/e2e/scenarios.py','download-response-cut',timeout=300)
                record['subcases']=sdk_subcases_from_output(output)
                for node in self.nodes:self.helper(node,'empty',node)
        if 'schedule-deadline' in selected:
            with self.case('schedule-deadline', checks) as record:
                output=self.execute('node1','/opt/adx/client/bin/python','-u',
                                    '/opt/adx/e2e/scenarios.py','schedule-deadline',timeout=180)
                record['subcases']=sdk_subcases_from_output(output)
                for node in self.nodes:self.helper(node,'empty',node)
        ...
        for scenario,role in (('apiserver-restart','apiserver'),('ingress-restart','ingress')):
            ...
            with self.case(scenario, checks):
                self.event('Create pinned instances, restart '+role+' and verify public SDK routing')
                self.execute('node1','/opt/adx/client/bin/python','-u','/opt/adx/e2e/scenarios.py','create-marker',timeout=300)
                for node in self.nodes:self.helper(node,'capture-backend',node)
                self.helper('node1','gateway-restart',role,timeout=90)
                for node in self.nodes:self.helper(node,'unchanged',node)
                self.execute('node1','/opt/adx/client/bin/python','-u','/opt/adx/e2e/scenarios.py','recovered-marker',timeout=90)
                self.execute('node1','/opt/adx/client/bin/python','-u','/opt/adx/e2e/scenarios.py','cleanup-live-gateway',role,timeout=90)
                for node in self.nodes:self.helper(node,'empty',node)
        if 'stop' in selected:
