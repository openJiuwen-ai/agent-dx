...
def run(connection, image, output):
    ...
    for name, options in [('default', {}), ('runtime-only', {'runtime':'runc'}), ('custom', {'image':image})]:
        ...
        try:
            command="test -x /__adx/usr/local/bin/adx-execd && printf 'runtime_profile-ready'"
            if name=='custom' and Path('/secrets/runtime-image').is_file():
                command="test ! -e /usr/local/bin/adx-execd && " + command
            result=s.commands.run(command)
            assert result.exit_code==0 and result.stdout=='runtime_profile-ready', (name,result)
            results.append({'mode':name,'id':s.id,'command':True})
            print('PASS runtime profile: '+name,flush=True)
        finally:
            try:s.kill()
            finally:s.close()
