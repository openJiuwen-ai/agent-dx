...
if sys.argv[1]=='l0':
    ...
    run(connection,image,result)
    print(json.dumps(json.loads(result.read_text())),flush=True)
elif sys.argv[1].startswith('loss-create-'):
    from sandboxd_runtime_loss import create
    ...
    print(json.dumps(report),flush=True)
elif sys.argv[1]=='command-watch-unavailable':
    from command_watch_unavailable import run
    report=run(connection,image,E/'command-watch-unavailable-result.json',S)
    print(json.dumps(report),flush=True)
elif sys.argv[1]=='command-watch-query-unavailable':
    from command_watch_unavailable import run
    report=run(connection,image,E/'command-watch-query-unavailable-result.json',S,
    ...
    print(json.dumps(report),flush=True)
elif sys.argv[1]=='download-response-cut':
    from download_response_cut import run
    report=run(connection,image,E/'download-response-cut-result.json',S)
    print(json.dumps(report),flush=True)
