...
本文使用统一发布包在一台 Linux 主机上部署 Coordinator、adxlet（默认内嵌 Relay）和 API Server（默认内嵌 Ingress）。入口是 HTTPS Ingress；API Server 的 HTTP 监听只绑定本机回环地址，内部 RPC 与 Ingress→Relay 默认使用 mTLS。Profile 可显式配置 `internal_security: network` 关闭内部 mTLS，对外 HTTPS 不变，详见部署指南。`adxctl` 的命令语义、按角色拆分部署、内置／外置 Redis 配置见独立的 [`adxctl` 部署指南](adxctl.md)。
...
- 配置随包部署的本地 `runtime/adx-runtime-rootfs.img`：默认用作实例根文件系统，自定义镜像时只读挂载内置环境，用户镜像不需要预装 Execd。配置及启动语义见 [本地运行环境](runtime-environment.md)。
...
检查配置中的包目录、状态目录、监听地址、Redis 地址和 namespace。示例采用单机回环地址发布内部服务；分节点部署时必须改为对端能访问的地址，并调整防火墙与 CIDR。对外 Ingress 默认监听 8443，仅允许本机客户端；远程客户端需要配置 `ADX_DATA_PLANE_INGRESS_ALLOWED_CLIENT_CIDRS`，证书 SAN 也需包含实际入口域名或 IP。
...
将 Relay 的 `ADX_DATA_PLANE_ALLOWED_TARGET_CIDRS` 改为 sandboxd 实际实例网络；示例 `10.88.0.0/16` 是部署输入，不是后端网络探测结果。
...
| 文件 | 使用方 |
| --- | --- |
| `tls/ca.pem` | 内部 RPC 及 Ingress→Relay 的 CA |
| `tls/coordinator.pem`、`coordinator.key`、`coordinator.der` | Coordinator 证书、私钥和供对端识别的叶证书 DER |
| `tls/node-1.pem`、`node-1.key`、`node-1.der` | adxlet 和本机 Relay |
| `tls/apiserver.pem`、`apiserver.key`、`apiserver.der` | API Server 内部 RPC 身份 |
| `tls/ingress.pem`、`ingress.key`、`ingress.der` | Ingress 内部 RPC 与代理客户端身份 |
| `tls/ingress-public.pem`、`ingress-public.key` | Ingress 对外 HTTPS；客户端信任其签发 CA |
| `tls/public-ca.pem` | SDK 及管理客户端信任的对外 HTTPS CA，可与内部 CA 不同 |
| `secrets/admin-key` | 初始管理员 API Key 明文文件，仅由 Coordinator 初始化读取 |

以上文件相对 `/opt/adx/config`。私钥和 API Key 权限设为 0600，目录设为 0700，并允许对应服务用户读取。部署环境可用 `openssl x509 -in component.pem -outform DER -out component.der` 从叶证书生成 DER；`peers` 映射检查叶证书字节，因此只更新 PEM 不足以完成内部身份轮换。`node:node-1` 必须与 adxlet 的 `node_id` 一致。

用密码学随机数生成初始管理员 API Key 并写入受保护文件。配置只引用路径。Coordinator 保存摘要；用管理员接口创建租户密钥，见 [API Key 管理](../testing/api-key-management.md)。正常业务客户端使用租户密钥。
...
CLI 为 API Server 注入共享 Redis 地址和 namespace，发现轮询间隔 `config.discovery.poll_seconds` 默认 5 秒，可配置为 1–86400 秒。adxlet 的发现配置采用自身协议，由 CLI 分别生成。

`validate` 校验部署结构和 CLI 约束；TLS 文件内容、组件字段、sandboxd 和网络连通性由组件启动与真实请求检查。sandboxd 可以晚于 supervisor 启动：adxlet 会保持存活并重复等待后端就绪，在此期间不会向 Coordinator 注册或接收新实例；配置错误仍会使进程退出。`render` 的输出目录必须尚不存在；生成目录 0700、文件 0600，可能包含 Redis 连接凭证，不应上传为公开日志。`run`/`start` 都在前台运行 supervisor，systemd 或 Pod 可直接托管该进程。
...
sandbox = Sandbox(image='YOUR_EXECD_IMAGE', runtime='runc', cpu=250, memory=256,
...
先安装包内 `sdk/` 下的 wheel。暂停/恢复需使用支持 checkpoint 的后端并准备制品目录或对象存储，不能用 runc 的基础创建验收替代，见 [快照与存储](../testing/snapshot-storage.md)。组件日志位于部署的 `state_dir/logs/`；就绪失败优先检查证书、Redis、资源源、sandboxd、Execd 与 Relay 绑定。
