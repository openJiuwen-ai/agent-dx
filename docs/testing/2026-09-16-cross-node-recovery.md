...
- 内部 `NodeService.RecoverInstance` 校验目标进程会话和归属，并进入已有的 Instance 串行控制任务。相同代次使用同一操作 ID，重复请求返回原结果。执行已成功而发布暂时失败时，只补提交结果。
...
1. Core：显式源身份允许同 Instance ID 转移到更高代次，拒绝相同或更低代次。
...
第4项使用真实对象存储适配器和内存对象存储，RuntimeBackend、就绪与路由执行端为测试实现。这是组件集成证据，不能替代双节点 Firecracker 或 Kubernetes 端到端验收。
