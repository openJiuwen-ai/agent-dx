...
调度层命名为 Scheduling Shard：`ShardScheduler`、`shard_id`、`scheduler_shards`。Global 轮转 → Shard Filter/Score → Node Manager 准入保持。CLI、证书身份、配置、统一包、K8s 部署和指标消费者同步迁移，具体升级步骤见 [迁移记录](rust-apiserver.md)。
