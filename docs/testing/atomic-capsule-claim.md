...
| 本地不足或硬约束不满足 | 释放可确认属于本次申请的暂留，使用原 Capsule ID/规格进入 ShardScheduler；中心排队和评分在这里生效 |
...
API Server：WatchNodes 全量目录 → 轮转入口
  → Node Manager：CreateLocalCapsule，检查租户、节点会话与生命周期准入门
  ...
  → Node Manager：暂留 token 转交 Assignment，复用该 Capsule 串行控制器
...
Master 的 `ForwardCreate` 仅接受受信节点调用；原有中心 `CreateCapsule` 仍接受 API Server。
...
共用 Redis 控制 header 和 Capsule 字段；generation 在 Rust 中按 `u64` 计算。
...
| `Unavailable`/超时 | 结果未知，保留 Capsule ID、规格、设备选择和暂留；不得换 ID 重建 |

同节点入口按 Capsule ID 串行化；不同调用共用同一暂留 token。标量资源和卡全部成功才更新 Admission。
...
入口 Node Manager 只有在本地资源或硬约束明确不满足时，才使用剩余整体预算向 Master 的中心调度提交**同一 ID 和规格**。API Server 不会在入口连接失败、超时或会话前置条件失效后自行发起第二条中心创建；这些错误可能对应已接受的本地操作，调用方必须使用同一请求和 Capsule ID 重试或查询。
Master 将中心请求与仍在途的 claim 串行协调。API 不把结果未知缓存成最终失败；保留的创建操作可继续使用原规格重试。`scheduleTimeoutSeconds` 仅约束中心队列等待 Assignment，本地入口使用完整的 `createTimeoutSeconds`，不做固定比例切分。
这里保证同 Capsule ID 的收敛，不提供跨 API Server 的匿名 HTTP 请求 ID 全局持久化服务。
...
- `build/ci/local_first_http.py`：实际 Rust API HTTPS → Node/Master RPC，目录轮转、4 个并发请求、不同 HTTP 请求 ID 复用同 Capsule、规格/租户冲突、删除后资源归零。
...
RPC/HTTPS fixture 中的 RuntimeDriver 是测试实现；真实 sandboxd/runc/RRT 新制品本地双节点8组验收见 [本地端到端报告](2026-09-17-local-first-e2e.md)。提交 `363e44f` 的 [Buildkite #30](2026-09-18-runtime-environment-k8s.md) 已使用正式发布包、OCI 镜像和 Kubernetes 部署完成同一八组验收。
