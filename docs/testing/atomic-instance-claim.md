...
入口 Node Manager 只有在本地资源或硬约束明确不满足时，才使用剩余整体预算向 Master 的中心调度提交**同一 ID 和规格**。API Server 不会在入口连接失败、超时或会话前置条件失效后自行发起第二条中心创建；这些错误可能对应已接受的本地操作，调用方必须使用同一请求和 Instance ID 重试或查询。
