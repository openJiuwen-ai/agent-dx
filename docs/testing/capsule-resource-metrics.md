...
| 指标 | 标签 | 含义 |
| --- | --- | --- |
| `adx_master_capsules` | shard_id, node_id, state | 已分配归属的 Capsule 当前状态数量，包含无执行结果的Reserved、失效的Invalidated和跨节点Recovering；Deleted不计入 |
| `adx_master_queued_requests` | shard_id | 尚未分配的内存队列长度；单列，不与 Capsule 计数相加解释为运行数量 |
...
CPU使用量等既有 `adx_capsule_*` 指标保持，由RuntimeDriver采样。使用量与reserved不同，不应互相替代；当前使用量指标带 `capsule_id` 和 `runtime_id`，无独立明细开关；采样年龄可诊断陈旧数据，完整采样失败计数尚未提供。
...
按域汇总示例：`sum by (shard_id, state) (adx_master_capsules)`；按集群汇总：`sum(adx_master_node_reserved_cpu_millis)`。Master与Node的资源视图分别使用，不叠加求和，避免同一分配计数两次。

## 验收

针对容量下降、释放、维护状态、缺失/不健康设备、Master恢复后预留恢复与队列数量增加回归。真实Redis/mTLS生命周期测试额外抓取HTTP端点，核对Running、排队、删除后的 Capsule 和资源；心跳故障测试核对Invalidated和不可用容量。结果及部署验收边界另行记录，组件/Socket测试不能代替正式K8s验证。
