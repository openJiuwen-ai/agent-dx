...
```text
可信 API Server → Master.CreateInstance
...
可信 API Server → Master.GetInstance
  ← 最后提交的实例结果 + Node Manager 地址
可信 API Server → 对应 Node.DeleteInstance
...
| Master.CreateInstance | API Server | CallerContext 与 Spec 租户匹配，或已认证管理员 |
| Master.GetInstance | API Server 或归属 Node | 租户权限／节点归属 |
...
| Node.DeleteInstance | API Server | 租户权限、本机控制器、完整 Assignment |
