# Coordinator Redis 持久化与恢复

2026-09-17 核对。Coordinator 存储、RPC、Node StateSink 和 Ingress 路由订阅均已接通；普通生命周期状态决策仍在 adxlet。下文末尾保留 2026-09-14 存储组件测试记录。

## 模块与边界

| 模块 | 职责 |
|---|---|
| `coordinator/src/storage.rs`：`RedisStore` | Redis 连接、超时、断线后重新连接、原子条件提交 |
| 同上：`Session` | Coordinator 启动代次、节点自动分域、分配持久化、节点结果核验与提交 |
| 同上：`StoredSnapshot` | 一致读取持久化目录，生成已提交的 Running 路由视图 |
| `Coordinator::restore/restore_with_config` | 恢复节点归属、代次下限、标量和设备占用；保留调度配置入口 |
...
实例和发布版本不存在两份独立提交的状态：路由视图从同一次全量读取中的节点与已提交 Running 结果生成。revision 是集群目录发布游标，允许包含不改变路由的更新；不是 adxlet 的实例 revision。当前没有持久化增量事件流，gRPC 全量／增量由服务层从已提交视图发布，重连从全量开始。

这个布局面向已选的单 Coordinator、单 Redis。全局头部 CAS 会串行化持久化写入，有限重试 32 次后返回暂不可用；此阶段没有测量其高并发吞吐。数据规模增大时，需要单独测量启动全量读取与提交争用，不能沿用调度内核基准作为存储性能证据。
...
cargo test --locked -p adx-coordinator -p adx-core -p adx-scheduling -j2
cargo clippy --locked -p adx-coordinator -p adx-core -p adx-scheduling --all-targets -j2 -- -D warnings
```
...
实际测试环境：macOS ARM64，官方 Redis 7.2.5 源码独立编译，Rust 1.97.1。本次 Redis 是测试依赖，没有据此确定统一产品发布包的 Redis 版本。源码包与二进制摘要见 `out/ci/coordinator-storage/provenance.json`（当次本地产物）。
