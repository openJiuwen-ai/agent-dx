...
- RRT 在解析 HTTP 头后创建 `rrt.http`，Capsule ID 作为属性。此 Span 覆盖 HTTP 操作处理；异步提交命令之后的用户进程运行时间尚不属于该 HTTP Span。

创建、执行、删除是独立请求，各有自己的 Trace，通过 Capsule ID 关联。SDK 目前没有新增自动事务级根 Span。
...
API 请求日志包含 Trace ID 和 Span ID；Node Manager 操作完成日志包含 `traceparent`。业务字段只记录操作、方法、路由模板、状态、Capsule ID等；Trace不添加API Key、请求正文、命令内容、文件内容或用户代码。传播器仅处理W3C Trace字段，不传播 baggage。
...
本地与Rust API Server重写后的 [Buildkite #24 正式验收](2026-09-17-rust-api-server-k8s.md) 已通过；统一实时队列满丢弃指标后置，详见 [事项清单](control-plane-remaining.json)。节点恢复后产生新的后台Trace，以Capsule/代次/状态关联，不承诺跨进程重启续接已结束的Span。
