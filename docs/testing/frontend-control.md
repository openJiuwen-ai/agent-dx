...
2026-09-17 核对：HTTP 兼容层已接入 Instance RPC，提供 Master、Node Manager 和 Go Sandbox API 服务入口。生命周期状态机仍在 Node Manager。下文契约按当前代码描述，末尾测试表保留 2026-09-15 当次证据。
...
Go 兼容层内部已有的消息编解码暂时保留，跨进程调用只使用新 `adx.control.v1` 协议。已接通暂停／恢复、快照、空闲回收、重启策略和 HTTP 亲和策略翻译。网络策略、挂载、入口继承、extra_config、独立资源上限、公开端口发布、每实例安全模式、reload 及创建 failover=true 明确拒绝；相等的资源 request/limit 可兼容。用户命令仍走 RRT HTTP 数据接口。Agent 路由保留，配置 Agent 服务地址时转发给上层 Agent 服务；未配置时返回暂不可用。
...
组件间必须使用部署提供的 mTLS。Master 按证书指纹识别 Frontend 与具体 Node；身份上下文仅从受信 Frontend 接收。管理员密钥创建／查询／吊销和 Edge 认证已接线，见 [API Key 管理](api-key-management.md)。证书更新通过重启组件生效，热重载后置。

## 服务装配

三个进程均使用 `--config /path/to/config.json`，未知配置项拒绝。配置示例见 `build/config/examples/`，文件路径按进程工作目录解释。Go 默认使用 HTTPS；`loopback_http=true` 仅允许字面回环地址供同机 Edge 转发。内部 gRPC 客户端校验配置的 CA 与服务端名称。
...
入口支持 Redis 注册发现，Frontend 的 Master／认证客户端共用动态 resolver，节点直达缓存继续保留。Node Proxy 全量同步、统一配置生成与 supervisor 已装配。`adxctl stop` 和 supervisor 的 SIGTERM/SIGINT 会执行本机实例清理；单独 Node Manager 故障重启走权威对账。
