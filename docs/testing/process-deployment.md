...
`platform/deployment` 中的 Rust `adxctl` 承载统一部署配置和轻量 supervisor；它不属于某个平面，配置和托管 Coordinator/API Server、Ingress/Relay/adxlet。adxlet 仍负责本机 Environment 停止清理。统一包同时携带控制面和数据面。本文描述现有进程托管、停止契约和出包；末尾保留早期组件测试。完整 SDK 基础 K8s 已通过 [Buildkite #21](2026-09-17-observability-k8s.md)。
...
| `platform/adxlet/src/admin.rs` | 本机已管理 Environment 的停止清理；保持 adxlet 的生命周期所有权 |
...
| `build/ci/process_smoke.py` | 从统一包启动真实 Redis／Coordinator，注入进程退出、检查新 epoch、确认干净停止 |
...
默认配置路径为 `/opt/adx/config/deployment.yaml`；可通过全局 `-c/--config` 或 `ADX_DEPLOYMENT_CONFIG` 修改。`config init` 默认生成带本地托管 Redis 的 standalone 配置，并支持 `standalone-external-redis`、`coordinator`、`node` 和 `ingress-api` profile。统一配置示例位于 `build/config/examples/`。`services` 选择本机角色；同一包可以部署 Coordinator 主机或工作节点。每个服务的原有细节放在 `config` 和 `env` 中；公共 Redis／namespace 和管理 socket 由 CLI 注入。当前校验覆盖部署结构、公共字段、Redis 和 socket 等约束；TLS 文件、资源观测及其他组件细节仍由对应服务执行最终校验。
...
1. 调用本机各 adxlet 的 Drain。Node 关闭新准入，等待已接收的串行操作，逐一退役绑定、删除运行时、提交 Deleted。
2. 每个删除结果必须为 `Published` 且不占资源。Coordinator 不可用、清理失败或只进入本地降级日志时，停止不成功，保留依赖供重试。
3. 所有本机节点清理成功后，按相反顺序停止子进程。单个进程终止超时会被强制终止，并使这次停止返回失败。

adxlet 未完成权威对账时，不能用空内存目录宣称清理完成。清理开始后保持 draining；失败可再次执行。作用域是本机已接收并管理的 Environment，不执行远端节点排空或迁移。完整 E2E 还需覆盖停止与 Coordinator 在途分配之间的竞争。

sandboxd 始终由部署环境独立托管，角色枚举不允许 supervisor 拉起它。Execd 随包交付到 `runtime/`，须进入实际实例环境；不会被当成宿主公共服务启动。Relay 和 Ingress 均支持默认共进程及显式分进程，见 [Relay 进程模式](relay-process-modes.md)与 [Ingress 进程模式](api-ingress-process-modes.md)。
...
这不是两节点完整平台验收：没有启动真实 sandboxd／Execd，也没有通过安装后的公开 SDK 完成创建—命令—文件—删除。后续环境驱动器、实例镜像、资源源与 SDK 用例已接通，正式结果见本文开头。不会用本阶段的包构建或 Coordinator smoke 代替该门禁。
...
|---|---|---|
| CLI／Node／Coordinator／Protocol | 101 通过、0 失败、12 默认忽略 | `rust-final.log` |
...
| 真实包 Redis／Coordinator 重启与停止 | Coordinator 重启 1 次，epoch 1→2，干净停止 | `smoke-final/result.json` |
