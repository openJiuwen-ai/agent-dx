...
节点每次启动生成 `session_id`，每次报告递增 `heartbeat_sequence`。相同序号且相同请求可重试，但不会延长存活期限；倒退序号或同序号不同内容被拒绝。不同进程身份不能替换尚在心跳有效期内的节点。

Master 使用单调时钟判断心跳是否过期，`heartbeat_timeout_seconds` 默认 30 秒。过期后关闭该节点的新调度，并从持久化路由视图排除它；**保留已有分配和资源占用**。维护开关／容量不足与节点是否可路由分别存储，因此停止新调度不等同于下线已有实例。本阶段最初验证的是 Master 路由视图；后续的 [路由发布阶段](route-publication.md) 已接入 Edge 全量／增量订阅。

节点回来后重新进入对账握手；Master 重启后同样要求重新对账。较长的对账期间继续发送关闭准入的心跳。CommitInstance 校验节点证书、当前进程 session、完整 Assignment、Spec 和 revision。Master 派发 Create 也携带目标 Node session，节点拒绝发送给旧进程身份的迟到请求。

这里的 session 拒绝的是旧进程状态写入，不是对旧操作系统进程的物理隔离。本阶段不进行跨节点接管、不释放超时节点占用，也不从 checkpoint 自动恢复。后续启用接管前仍须完成运行隔离契约。

## 本机恢复规则

没有 Master 完整目录或无法读取实际运行时清单时，保持生命周期入口关闭；查询错误不能转换成空目录。启动时不依赖 SQLite 中的完整实例目录，当前也尚未实现 SQLite 降级日志。若 Master 的预留／注册持久化结果不确定而进入恢复保护，仍需重启 Master，从 Redis 重建调度内存；地址重连不清除这类一致性保护。
