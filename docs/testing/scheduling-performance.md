...
请求数、时间和日志容量须大于零。当前为 Rust 配置接口，部署 CLI 的配置装配随控制面服务接线完成。
...
当前工作树、macOS arm64、Rust release 模式结果：
...
已完成同一 Linux ARM64 容器内的历史基准二进制与当前 ADX 工作树复测。1,000 节点、Pack、七轮中位数：开启聚合／候选复用的持续调度与上报确认闭环，旧侧 14,219 QPS、ADX 21,180 QPS；生命周期 P99 为 373.46 → 236.24 ms。关闭复用时 13,937 → 14,171 QPS，基本持平。
