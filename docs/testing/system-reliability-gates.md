...
| SD-03 | daemon 重启且 runtime 丢失，Never | 实例进入 Failed、撤路由、释放资源，不自动冷启动 | 对账组件测试已验证该行为；[Full #39](https://buildkite.com/agent-dx/agent-dx-full-test/builds/39) 验证真实 backend 消失后的 Never 结果。daemon 自身重启并丢失 runtime 的组合仍待独立注入 |
| SD-04 | daemon 重启且 runtime 丢失，自动重启 | 遵守重试上限与退避，新 backend 使用同一 Environment ID 和有效 generation | 生命周期组件测试覆盖退避和上限；[Full #39](https://buildkite.com/agent-dx/agent-dx-full-test/builds/39) 验证真实 backend 连续消失与新执行身份。daemon 自身重启并丢失 runtime 的组合仍待独立注入 |
