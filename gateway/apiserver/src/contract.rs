//! Public JSON contract converted directly to typed Environment RPC payloads.
...
pub fn create_spec(body: Value, caller: &pb::CallerContext) -> Result<pb::EnvironmentSpec, Status> {
...
pub fn create_spec_with_environment(
    ...
    if r.env
        .keys()
        .any(|k| k.starts_with("ADX_") || k == "EXECD_HTTP_TOKEN")
    {
    ...
    let restart = r
        ...
        .transpose()?;
    // Forwarded ports do not allocate host ports: Ingress and Relay route
    // authenticated traffic directly to the Environment IP. Keep the public
    ...
    let readonly = root
        .readonly
        .or_else(|| runtime_profile.map(|value| value.rootfs.readonly))
        .unwrap_or(false);
    let rootfs =
    ...
    r.env.insert("EXECD_HTTP_PORT".into(), "50090".into());
    for (key, default) in [
        ("EXECD_COMMAND_RESULT_TTL_SECS", "3600"),
        ("EXECD_COMMAND_STDOUT_LIMIT_BYTES", "4194304"),
        ("EXECD_COMMAND_STDERR_LIMIT_BYTES", "4194304"),
        ("EXECD_COMMAND_REGISTRY_MAX_RECORDS", "4096"),
        ("EXECD_COMMAND_REGISTRY_MAX_BYTES", "268435456"),
        (
            "EXECD_COMMAND_REGISTRY_MEMORY_HIGH_WATERMARK_BYTES",
            "201326592",
        ),
        ("EXECD_COMMAND_ACTIVITY_HEARTBEAT_SECS", "10"),
        ("EXECD_COMMAND_WATCH_MAX_SUBSCRIPTIONS", "4096"),
        ("EXECD_COMMAND_WATCH_MAX_FRAME_BYTES", "1048576"),
    ] {
        let name = match key {
            "EXECD_COMMAND_WATCH_MAX_SUBSCRIPTIONS" => {
                "ADX_COMMAND_WATCH_MAX_SUBSCRIPTIONS_PER_CONNECTION".into()
            }
            "EXECD_COMMAND_WATCH_MAX_FRAME_BYTES" => "ADX_COMMAND_WATCH_MAX_FRAME_BYTES".into(),
            _ => format!("ADX_{key}"),
        };
        r.env.insert(
            key.into(),
    ...
    if r.tunnel.enabled {
        ...
        r.env
            .insert("EXECD_TUNNEL_HTTP_PORT".into(), port.to_string());
        r.env
            .insert("EXECD_TUNNEL_WS_PORT".into(), (port - 1).to_string());
    }
    let runtime_profile = if snapshot.is_none() {
        runtime_profile.cloned().map(|mut value| {
...
fn affinities(
    ...
    for a in input {
        ...
        let target = if a.kind == 0 {
            ...
            pb::PlacementTarget::Environment
        } as i32;
