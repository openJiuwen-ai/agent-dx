//! Real SSH and NodeProxy sockets with a fake Platform lifecycle backend.
use super::*;
use crate::{
    common::protocol::GatewayPolicy,
    edge::ssh::{KeyGrant, SshConfig, SshListener},
    node::NodeProxy,
};
use adx_agent_core::{
...
async fn terminal_case(native: bool) {
    ...
    let edge = tokio::spawn(listener.serve(stopped));
    let checks = async {
        let target = SshRoute {
