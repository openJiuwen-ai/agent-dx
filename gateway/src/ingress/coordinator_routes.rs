...
use crate::common::route::{
    DataPlaneAuthMode, DataPlaneSecurityMode, EnvironmentStatus, PortForwardRoute, RouteCache,
    RouteInfo,
};
    ...
    pub fn apply(&mut self, f: pb::RouteFrame) -> Result<(), String> {
        ...
        for r in f.upserts {
            if r.environment_id.is_empty()
                ...
                || r.generation == 0
                || r.environment_revision == 0
                || !adx_protocol::valid_runtime_id(&r.environment_id, r.generation, &r.runtime_id)
                || r.runtime_ip.parse::<std::net::IpAddr>().is_err()
                || r.relay_address.contains(['/', '@', '?', '#'])
                || r.relay_address
                    .parse::<http::uri::Authority>()
                    .ok()
                    .is_none_or(|a| a.port_u16().is_none())
                || !ids.insert(r.environment_id.clone())
            {
                return Err("invalid published route".into());
            }
            if let Some(old) = self.routes.get(&r.environment_id) {
                if (r.generation, r.environment_revision)
                    < (old.generation, old.environment_revision)
                {
...
impl std::fmt::Debug for CoordinatorConnection {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str("CoordinatorConnection")
    }
}
impl CoordinatorConnection {
    pub fn new(c: ControlConfig) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let (_, tls, _) = c.tls.load_rpc(adx_protocol::auth::Principal::Ingress)?;
    ...
    pub async fn run(self: Arc<Self>, store: Arc<RouteStore>) {
        ...
        loop {
            if self.subscribe(&mut consumer).await.is_err() {
                tracing::warn!(
                    "Coordinator route subscription interrupted; retaining synchronized cache"
                );
            }
...
impl CredentialVerifier for CoordinatorConnection {
    async fn verify(&self, key: &str) -> Result<AuthenticatedIdentity, AuthError> {
        ...
        let channel =
            self.channel.read().unwrap().clone().ok_or_else(|| {
                AuthError::IamUnavailable("Coordinator connection not ready".into())
            })?;
        ...
        .await
        .map_err(|_| AuthError::IamUnavailable("Coordinator deadline".into()))?
        .map_err(|s| {
            if s.code() == tonic::Code::Unauthenticated {
                AuthError::Invalid("API key rejected".into())
            } else {
                AuthError::IamUnavailable("Coordinator verification unavailable".into())
            }
        })?
        .into_inner();
        let caller = result
