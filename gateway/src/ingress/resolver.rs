...
use crate::common::route::{
    DataPlaneAuthMode, DataPlaneSecurityMode, EnvironmentStatus, RouteInfo,
};
use crate::ingress::route_store::{RouteChange, RouteStore};
...
#[derive(Clone)]
pub struct IngressRouteResolver {
...
impl IngressRouteResolver {
    ...
    pub fn route_is_current(&self, handle: &RouteHandle) -> bool {
        self.store
            ...
            .is_some_and(|route| {
                is_connectable(&route.environment_status)
                    && route.relay_address == handle.relay_address
    ...
    pub async fn resolve(
        ...
        if !is_connectable(&route.environment_status) {
            return Err(ResolveError::EnvironmentStatus {
                code: route.environment_status.code,
                msg: route.environment_status.msg.clone(),
                err_code: route.environment_status.err_code,
                exit_code: route.environment_status.exit_code,
                kind: route.environment_status.kind,
            });
        }
        if route.relay_address.is_empty() {
        ...
        Ok(RouteHandle {
            access_kind,
            relay_address: route.relay_address,
...
fn is_connectable(status: &EnvironmentStatus) -> bool {
    status.code == 3
}
...
mod tests {
    ...
    async fn all_access_kinds_share_one_l4_target() {
        ...
        let resolver = IngressRouteResolver::new(store);
        for kind in [
            AccessKind::Direct,
            AccessKind::Tunnel,
    ...
    #[tokio::test]
    async fn preserves_environment_status_error() {
        let store = Arc::new(RouteStore::new());
        store.put(route(5));
        store.set_ready(true);
        let resolver = IngressRouteResolver::new(store);
        assert!(matches!(
            ...
            Err(ResolveError::EnvironmentStatus { code: 5, .. })
        ));
    }
}
