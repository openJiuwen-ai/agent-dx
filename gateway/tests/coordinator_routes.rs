...
use data_plane_gateway::ingress::{coordinator_routes::RouteConsumer, RouteStore};
use std::sync::Arc;
...
async fn stream_routes_require_auth_and_missing_cache_never_point_gets() {
    use data_plane_gateway::{
        ...
        ingress::{AccessKind, IngressRouteResolver, ResolveError},
    };
    let store = Arc::new(RouteStore::new());
    let mut c = RouteConsumer::new(store.clone());
    c.apply(RouteFrame {
...
async fn stream_only_resolver_honors_per_instance_tls_without_token() {
    use data_plane_gateway::{
        common::route::DataPlaneAuthMode,
        ingress::{AccessKind, IngressRouteResolver},
    };
    let store = Arc::new(RouteStore::new());
    let mut consumer = RouteConsumer::new(store.clone());
    let mut published = route("tls");
    ...
    let resolved = IngressRouteResolver::new(store)
        .stream_only()
...
fn resumed_execution_route_replaces_source_and_rejects_older_revision() {
    ...
    restored.environment_revision = 6;
    c.apply(RouteFrame {
        epoch: 1,
        revision: 2,
        base_revision: 1,
