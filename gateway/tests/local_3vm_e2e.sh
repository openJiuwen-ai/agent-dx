...
copy_to "${bin_dir}/adx-edge-frontend" "${master}" "${remote_root}/bin/adx-edge-frontend"
copy_to "${bin_dir}/adx-data-plane-forward" "${master}" "${remote_root}/bin/adx-data-plane-forward"
...
for node in "${worker1}" "${worker2}"; do
    copy_to "${bin_dir}/adx-node-proxy" "${node}" "${remote_root}/bin/adx-node-proxy"
...
openssl req -x509 -newkey rsa:2048 -nodes -days 1 -subj /CN=adx-edge.local \
    -addext "subjectAltName=DNS:adx-edge.local,IP:127.0.0.1,IP:${master_ip}" \
...
start_node() {
    local node=$1
    remote "${node}" "
        chmod 0755 ${remote_root}/bin/adx-node-proxy
        nohup env \
          ADX_DATA_PLANE_NODE_PROXY_BIND=0.0.0.0:8443 \
          ADX_DATA_PLANE_NODE_PROXY_ADVERTISE_ADDRESS=\$(hostname -I | awk '{print \$1}'):8443 \
          ADX_DATA_PLANE_NODE_PROXY_HEALTH_BIND=0.0.0.0:18443 \
          ADX_DATA_PLANE_ALLOWED_TARGET_CIDRS=10.88.0.0/16 \
          ADX_DATA_PLANE_ALLOWED_EDGE_CIDRS=${master_ip}/32,127.0.0.1/32 \
          ...
          RUST_LOG=info \
          ${remote_root}/bin/adx-node-proxy >${remote_root}/logs/node-launcher.log 2>&1 &
...
remote "${master}" "${remote_root}/bin/etcdctl --endpoints=http://127.0.0.1:2379 put /adx/route/business/adxk/vm-sandbox-1 '${route1}' >/dev/null"
remote "${master}" "${remote_root}/bin/etcdctl --endpoints=http://127.0.0.1:2379 put /adx/route/business/adxk/vm-sandbox-2 '${route2}' >/dev/null"

remote "${master}" "
    nohup env \
      ADX_DATA_PLANE_EDGE_FRONTEND_ETCD_ENDPOINTS=http://${master_ip}:2379 \
