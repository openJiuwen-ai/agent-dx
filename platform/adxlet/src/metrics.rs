...
impl Metrics {
    pub fn record(&self, environment: &str, runtime: &str, usage: RuntimeUsage) {
        self.0
            .lock()
            .expect("shared state lock poisoned")
            .insert(environment.into(), (runtime.into(), usage, Instant::now()));
    }
    pub fn remove(&self, environment: &str) {
        self.0
            .lock()
            .expect("shared state lock poisoned")
            .remove(environment);
    ...
    fn render(&self) -> String {
            ...
            self.0.lock().expect("shared state lock poisoned").iter()
        {
            ...
            for (name, value) in [
                (
                    "adx_environment_cpu_usage_seconds_total",
                    usage.cpu_usage_ns as f64 / 1e9,
                ),
                (
                    "adx_environment_memory_usage_bytes",
                    usage.memory_usage_bytes as f64,
                ),
                (
                    "adx_environment_memory_limit_bytes",
                    usage.memory_limit_bytes as f64,
                ),
                (
                    "adx_environment_stats_age_seconds",
...
pub async fn serve(manager: Arc<Adxlet>, bind: std::net::SocketAddr) -> std::io::Result<()> {
    use bytes::Bytes;
