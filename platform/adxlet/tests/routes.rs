//! Real Adxlet client -> real Relay gRPC handler over local UDS.
use adx_core::{
    Assignment, EnvironmentRecord, EnvironmentSpec, EnvironmentState, Error, Resources,
};
use adxlet::{routes::UdsRoutes, Routes};
...
fn service() -> Arc<BindingService> {
    Arc::new(BindingService::new(Arc::new(
        Relay::new(GatewayPolicy::new(vec![])).with_route_enforcement(),
    )))
}
struct Server {
...
async fn start<T: RelayService>(service: T) -> (Server, UdsRoutes) {
...
async fn startup_snapshot_gates_traffic_and_proxy_restart_replays_local_catalog() {
    let proxy = Arc::new(Relay::new(GatewayPolicy::new(vec![])).with_route_enforcement());
    ...
    let replacement = Arc::new(Relay::new(GatewayPolicy::new(vec![])).with_route_enforcement());
    let new = Arc::new(BindingService::new(replacement.clone()));
    let new_handler = new.clone();
    let listener = UnixListener::bind(path).unwrap();
    server.task = tokio::spawn(async move {
        tonic::transport::Server::builder()
            .add_service(RelayServiceServer::new(Handler(new_handler)))
            .serve_with_incoming(UnixListenerStream::new(listener))
            .await
            .unwrap();
    });
