...
| `same_operation` | 只能使用相同 Request ID、Operation ID 和 Capsule ID 查询或重试，不能换 ID 重建 |
...
创建请求未显式提供名称时，SDK 使用 `create-*` Request ID 中的 UUID 生成稳定名称。跨 API Server 重试因此仍得到相同 Capsule ID；查询暂时返回 404 不能触发换名称或创建第二个实例。
