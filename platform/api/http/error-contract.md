...
本文定义 API Server、内部 gRPC 和 Sandbox SDK 之间稳定的错误语义。适用范围是 Sandbox 创建、查询、暂停、恢复、快照、删除、实例目录查询及管理员接口。Edge → Node Proxy → RRT 的应用流量错误属于数据面协议，不使用本文的自动重试规则。
...
| `same_operation` | 只能使用相同 Request ID、Operation ID 和 Instance ID 查询或重试，不能换 ID 重建 |
...
同一个 gRPC 状态可能映射成不同稳定码。例如 `Unavailable` 在写请求尚未提交时是 `UNAVAILABLE`，在 Node Manager 已经接受创建或生命周期操作后是 `OUTCOME_UNKNOWN`。API Server 根据请求是否越过执行边界作出映射；SDK 不重新猜测。
...
创建请求未显式提供名称时，SDK 使用 `create-*` Request ID 中的 UUID 生成稳定名称。跨 API Server 重试因此仍得到相同 Instance ID；查询暂时返回 404 不能触发换名称或创建第二个实例。

实现位置：

- API Server 分类与序列化：`platform/control-plane/api-server/src/errors.rs`、`src/http.rs`
