...
impl pb::snapshot_service_server::SnapshotService for MasterRpc {
    async fn publish_snapshot(
        ...
        let source = state
            ...
            .ok_or_else(|| Status::failed_precondition("snapshot source is not assigned"))?;
        ...
        match state.session.get_snapshot(&snapshot.id).await {
            Ok(existing)
                if existing.same_content(&snapshot) && existing.state == SnapshotState::Ready =>
            {
    ...
    async fn list_snapshots(
        ...
        let mut page: Vec<_> = snapshots
            .into_iter()
            .filter(|s| s.id > r.page_token && (r.name.is_empty() || s.names.contains(&r.name)))
            .take(size + 1)
            .collect();
