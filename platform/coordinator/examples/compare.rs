...
use adx_core::{Assignment, EnvironmentSpec, Resources};
use std::{
...
fn spec(i: usize) -> EnvironmentSpec {
    EnvironmentSpec {
...
fn coordinator(cache: usize, capacity: u64) -> Coordinator {
    let mut m = Coordinator::with_config(
        1,
        Placement::Pack,
        SchedulerConfig {
...
enum Command {
    Schedule(EnvironmentSpec, Sender<Assignment>),
...
fn handle(
    ...
    match cmd {
        ...
        Command::Retry(a, tx) => {
            pending.insert(a.environment_id.clone(), tx);
            m.retry(&a).unwrap();
        }
        Command::Add(a, tx) => {
            let snapshot = m.snapshot();
            let p = &snapshot.environments()[&a.environment_id];
            assert_eq!(p.node_id, a.node_id);
            assert!(confirmed.insert(a.environment_id));
            tx.send(()).unwrap();
        }
        Command::Delete(a, tx) => {
            assert!(confirmed.remove(&a.environment_id));
            m.release(&a).unwrap();
            tx.send(()).unwrap();
        }
        Command::Update(n, tx) => {
        ...
        Command::Stop(tx) => {
            ...
            tx.send(m.snapshot().environments().len()).unwrap();
            return false;
        }
    }
    true
...
impl Actor {
    fn new(cache: usize, capacity: u64) -> Self {
        let mut m = coordinator(cache, capacity);
            ...
            loop {
                ...
                if let Some(d) = ready.or_else(|| m.take_ready_shard()) {
                    ...
                    for a in r.assignments {
                        pending.remove(&a.environment_id).unwrap().send(a).unwrap();
                    }
                }
...
fn print(
    ...
    println!("ADX_COMPARE {{\"case\":\"{case}\",\"cache\":{cache},\"nodes\":{NODES},\"requests\":{count},\"qps\":{},\"p50_us\":{},\"p99_us\":{},\"report_p50_us\":{},\"report_p99_us\":{},\"final_environments\":{final_count},\"updates\":{updates},\"success\":{count},\"invalid_placement\":0}}",count as f64/elapsed.as_secs_f64(),percentile(latency,50),percentile(latency,99),percentile(report,50),percentile(report,99));
}
fn main() {
    ...
    let count: usize = std::env::var("DOMAIN_BENCH_REQUEST_COUNT")
        ...
        .unwrap();
    if case == "direct" {
        let mut m = coordinator(cache, (count + 30) as u64);
        for i in 0..30 {
