...
fn submit(
    ...
    priority: i32,
    tick: usize,
    pending: &mut BTreeMap<String, (EnvironmentSpec, usize, usize)>,
) {
    ...
    let shard = coordinator.submit(request.clone()).unwrap();
...
fn run(waves: usize, placement: Placement, cache: usize) -> Value {
    ...
    for n in 0..DOMAINS * 2 {
        assert_eq!(coordinator.register(node(n, true)).unwrap(), n % DOMAINS);
    }
    ...
    for wave in 0..waves {
        ...
        coordinator.register(node(unavailable, false)).unwrap();
        ...
        for tick in 0..256 {
            if tick == 3 {
                coordinator.register(node(unavailable, true)).unwrap();
            }
            ...
            for _ in 0..DOMAINS {
                ...
                let outcome = coordinator.schedule_round(shard).unwrap();
                assert!(outcome.error.is_none());
                assert!(outcome.attempted <= 5);
                rounds += 1;
                for assignment in outcome.assignments {
                    let (request, expected_shard, queued_at) =
                        pending.remove(&assignment.environment_id).unwrap();
                    ...
                    digest.update(format!(
                        "{}:{}:{}\n",
                        assignment.environment_id, assignment.node_id, tick
                    ));
        ...
        assert!(coordinator.snapshot().environments().is_empty());
        assert!(usage.iter().all(|u| *u == Resources::default()));
        for shard in 0..DOMAINS {
            assert_eq!(coordinator.pending(shard).unwrap(), 0);
        }
        if (wave + 1) % 250 == 0 {
            eprintln!(
    ...
    let stats: Vec<_> = (0..DOMAINS)
        .map(|d| coordinator.stats(d).unwrap())
        .collect();
