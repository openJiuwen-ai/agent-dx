...
use adx_core::{Assignment, EnvironmentSpec, Error, ResourceLedger, Result};
...
impl From<&EnvironmentSpec> for Signature {
    fn from(r: &EnvironmentSpec) -> Self {
        Self {
            image: r.image.clone(),
...
impl ShardScheduler {
    ...
    pub fn metrics(
        ...
        let shard_labels = [("shard_id", shard.to_string())];
        out.gauge(
            "adx_coordinator_queued_requests",
            &shard_labels,
            self.pending() as u64,
        );
        for (id, state) in &self.nodes {
            ...
            out.gauge(
                "adx_coordinator_node_schedulable",
                &labels,
                u64::from(accepting),
            );
            adx_observability::metrics::resources(
                out,
                "adx_coordinator_node",
                &labels,
                &state.scalar,
                &state.devices,
                accepting,
    ...
    pub fn restore(&mut self, spec: &EnvironmentSpec, assignment: &Assignment) -> Result<()> {
    ...
    pub fn enqueue(&mut self, request: EnvironmentSpec) {
    ...
    pub fn attempt(
        ...
        journal: &MutationJournal,
        config: &SchedulerConfig,
    ) -> Result<Option<(Assignment, EnvironmentSpec)>> {
    ...
    pub fn retry(&mut self, assignment: &Assignment, spec: EnvironmentSpec) -> Result<()> {
        ...
        self.excluded
            .insert(assignment.environment_id.clone(), excluded);
        self.enqueue(spec);
        Ok(())
    }
    pub fn release(&mut self, assignment: &Assignment) -> Result<()> {
        if self.assigned.get(&assignment.environment_id) != Some(assignment) {
            return Err(Error::Conflict);
        }
        let state = self
            .nodes
            .get_mut(&assignment.node_id)
            .ok_or(Error::NotFound)?;
        state.scalar.release(&assignment.environment_id)?;
        state.devices.release(&assignment.environment_id)?;
        if !assignment.devices.is_empty() {
            state.free_devices = state.devices.available();
        }
        self.assigned.remove(&assignment.environment_id);
        self.excluded.remove(&assignment.environment_id);
        Ok(())
    }
}
