use adx_coordinator::{Coordinator, Placement, SchedulerConfig};
use adx_core::{scheduling::*, EnvironmentSpec, Error, Resources, Result};
...
fn unsafe_policies_and_custom_profiles_never_enter_semantic_cache() {
    ...
    .unwrap();
    let mut m = Coordinator::with_framework(1, f).unwrap();
    m.register(node("a")).unwrap();
    for i in 0..3 {
...
fn cached_scalar_request_respects_new_cross_shard_reverse_anti_affinity() {
    let mut m = Coordinator::new(2, Placement::Pack).unwrap();
    let mut a = node("a");
...
fn partial_round_failure_returns_committed_assignments_and_keeps_failed_request() {
    ...
    assert_eq!(m.snapshot().environments().len(), 1);
    ...
    assert_eq!(m.snapshot().environments().len(), 2);
}
#[test]
fn time_budget_yields_between_requests_without_losing_work() {
    ...
    let mut m = Coordinator::with_framework_config(1, f, config).unwrap();
    m.register(node("a")).unwrap();
    for i in 0..3 {
        m.submit(spec(&i.to_string())).unwrap();
    }
...
fn invalid_limits_are_rejected() {
    for c in [
        ...
        assert!(Coordinator::with_config(1, Placement::Pack, c).is_err());
    }
}
