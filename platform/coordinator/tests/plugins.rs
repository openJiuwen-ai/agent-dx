use adx_coordinator::{Coordinator, Node};
...
impl Filter for ExcludeA {
    ...
    fn filter(&self, _: &EnvironmentSpec, node: &Candidate<'_>) -> Result<bool> {
        Ok(node.node.id != "a")
    }
}
struct Prefer {
...
impl Score for Prefer {
    ...
    fn score(&self, _: &EnvironmentSpec, candidate: &Candidate<'_>) -> Result<u32> {
        assert!(
...
fn weights_change_selection_and_ties_are_deterministic() {
    for (weight, expected) in [(1, "a"), (3, "b")] {
        ...
        let mut coordinator = Coordinator::with_framework(1, framework).unwrap();
        for id in ["b", "a"] {
            coordinator.register(node(id, true)).unwrap();
        }
        coordinator.submit(spec("i")).unwrap();
...
fn plugin_error_does_not_drop_queued_requests_or_reserve_capacity() {
    ...
    .unwrap();
    let mut coordinator = Coordinator::with_framework(1, framework).unwrap();
    coordinator.register(node("a", true)).unwrap();
    coordinator.submit(spec("first")).unwrap();
    coordinator.submit(spec("second")).unwrap();
    assert!(matches!(
    ...
    let first = coordinator.schedule(0).unwrap().unwrap();
    assert_eq!(first.environment_id, "first");
    coordinator.release(&first).unwrap();
    assert_eq!(
        coordinator.schedule(0).unwrap().unwrap().environment_id,
        "second"
