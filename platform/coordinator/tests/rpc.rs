mod common;
use adx_coordinator::{routes::RoutePublisher, rpc::CoordinatorRpc, storage::Session, Placement};
use adx_core::{
    scheduling::DeviceAllocation, EnvironmentRecord, EnvironmentSpec, Resources, Result,
};
use adx_protocol::{
...
use adxlet::{
    ...
    Adxlet, Readiness, Routes, RuntimeDriver,
};
use std::{
...
fn create(id: &str) -> pb::CreateEnvironmentRequest {
...
impl RuntimeDriver for Backend {
    async fn inventory(&self) -> Result<Vec<adxlet::RuntimeObservation>> {
        Ok(self
            ...
            .iter()
            .map(|id| {
                let (environment, generation) = id.rsplit_once('-').unwrap();
                adxlet::RuntimeObservation {
    ...
    async fn restore_from(
        ...
        if let Some(origin) = origin {
            assert_ne!(origin.environment_id, spec.id);
            let snapshot = self
                .session
...
impl Routes for LocalChecks {
    async fn retire_orphan(&self, _: &adxlet::RuntimeObservation) -> Result<()> {
        Ok(())
    }
...
async fn lifecycle_rpc_persists_before_execution_and_retries_only_the_result() {
    ...
    let auth = adx_coordinator::auth::AuthRpc::new(session.clone(), peers());
    let coordinator = CoordinatorRpc::new(
        session.clone(),
        Placement::Pack,
        peers(),
    ...
    .await
    .unwrap();
    let metrics_rpc = coordinator.clone();
    ...
    let mut servers = Servers(vec![
        ...
        tokio::spawn(async move {
            Server::builder()
                ...
                .add_service(pb::coordinator_service_server::CoordinatorServiceServer::new(coordinator))
    ...
    servers.0.push(tokio::spawn(async move {
        adx_coordinator::metrics::serve(metrics_listener, metrics_rpc)
            .await
            .unwrap();
    }));
    ...
    let sink = Arc::new(
        CoordinatorStateSink::new(channel(ma, "node").await, Duration::from_secs(2))
            .unwrap()
            .with_session("boot-1".into()),
    );
    let checkpoint_root = tempfile::tempdir().unwrap();
    let checkpoint_store = Arc::new(
        adxlet::checkpoint::LocalCheckpointStore::new(checkpoint_root.path().into()).unwrap(),
    );
    let manager = Arc::new(
        Adxlet::new(
            "node".into(),
    ...
    node_coordinator
        ...
        .await
        .unwrap();
    let mut frontend = pb::coordinator_service_client::CoordinatorServiceClient::new(
        channel(ma, "apiserver").await,
    );
    let mut duplicate = frontend.clone();
    let (a, b) = tokio::join!(
        frontend.create_environment(create("first")),
        duplicate.create_environment(create("first"))
    );
    ...
    assert!(metrics.contains(
        "adx_coordinator_environments{shard_id=\"0\",node_id=\"node\",state=\"Running\"} 1\n"
    ));
    assert!(metrics.contains(
        "adx_coordinator_node_reserved_cpu_millis{shard_id=\"0\",node_id=\"node\"} 100\n"
    ));

    let mut stale_coordinator =
        pb::node_service_client::NodeServiceClient::new(channel(na, "coordinator").await);
    assert_eq!(
        stale_coordinator
            .create_environment(pb::StartAssignedEnvironmentRequest {
                snapshot: None,
    ...
    let get = pb::GetEnvironmentRequest {
        environment_id: "first".into(),
        caller: caller(),
    };
    assert_eq!(
        frontend
            .get_environment(get.clone())
            .await
            .unwrap()
            .into_inner()
            .node_address,
    ...
    assert_eq!(
        frontend.get_environment(other).await.unwrap_err().code(),
        tonic::Code::PermissionDenied
    );
    let mut unknown =
        pb::coordinator_service_client::CoordinatorServiceClient::new(channel(ma, "unknown").await);
    assert_eq!(
        unknown.get_environment(get).await.unwrap_err().code(),
        tonic::Code::PermissionDenied
    );
    assert_eq!(
        frontend
    ...
    let mut node_frontend =
        pb::node_service_client::NodeServiceClient::new(channel(na, "apiserver").await);
    assert_eq!(
        node_frontend
            .create_environment(pb::StartAssignedEnvironmentRequest {
                snapshot: None,
    ...
    let mut pause = pb::PauseEnvironmentRequest {
        assignment: Some(assignment.clone()),
        caller: caller(),
    ...
    assert_eq!(
        node_frontend
            .pause_environment(pause)
            .await
            .unwrap_err()
            .code(),
        tonic::Code::PermissionDenied
    );
    let mut resume = pb::ResumeEnvironmentRequest {
        assignment: Some(assignment.clone()),
        caller: caller(),
    ...
    assert_eq!(
        stale_coordinator
            .resume_environment(resume.clone())
            .await
            .unwrap_err()
            .code(),
        tonic::Code::PermissionDenied
    ...
    assert!(frontend.create_environment(pending).await.is_err());
    assert!(scrape_metrics(metrics_address)
        .await
    ...
    node_frontend
        .delete_environment(delete.clone())
        .await
        .unwrap();
    node_frontend.delete_environment(delete).await.unwrap();
    ...
    if let Ok(api_binary) = std::env::var("ADX_TEST_APISERVER") {
        use adx_coordinator::auth::Credential;
        ...
        node_coordinator.register_node(update).await.unwrap();
...
#[tokio::test]
#[ignore = "requires isolated Redis and generated mTLS certificates; run control-rpc suite"]
async fn coordinator_process_loads_configuration_and_restores_bootstrap_credentials() {
    ...
    let key = "coordinator-process-test-key-".repeat(3);
    ...
    let config = serde_json::json!({
        ...
        "tls":{"ca":tls.join("ca.pem"),"certificate":tls.join("coordinator.pem"),"private_key":tls.join("coordinator.key"),"server_name":"localhost","peers":{"apiserver":tls.join("apiserver.der"),"node:node":tls.join("node.der")}},
    ...
    for index in 0..3 {
        let active_key = if index == 2 {
            "coordinator-rotated-key-".repeat(3)
        } else {
            key.clone()
        };
        ...
        let log = std::fs::File::create(
            PathBuf::from(std::env::var("ADX_TEST_EVIDENCE").unwrap())
                .join(format!("coordinator-process-{index}.log")),
        )
        .unwrap();
        let mut process = tokio::process::Command::new(env!("CARGO_BIN_EXE_adx-coordinator"))
            .args(["--config", path.to_str().unwrap()])
            .stdout(log.try_clone().unwrap())
            .stderr(log)
            .kill_on_drop(true)
        ...
        let connection = tokio::time::timeout(Duration::from_secs(5), async {
            loop {
                assert!(
                    ...
                    "Coordinator startup failed; inspect process log"
                );
                let endpoint =
                    tonic::transport::Endpoint::from_shared(format!("https://{address}"))
                        .unwrap()
                        .tls_config(client_tls("apiserver"))
                        .unwrap();
...
async fn heartbeat_expiry_reconciliation_and_old_session_fencing() {
    use adx_core::{Assignment, EnvironmentState};
    let redis = common::Redis::new().await;
    ...
    let mut node = pb::coordinator_service_client::CoordinatorServiceClient::new(
        channel(address, "node").await,
    );
    ...
    assert_eq!(catalog.coordinator_epoch, session.epoch());
    ...
    assert!(!snapshot.environments["held"].resources_held());
    let metrics = rpc.metrics().await.unwrap();
    assert!(metrics.contains(
        "adx_coordinator_environments{shard_id=\"0\",node_id=\"node\",state=\"Invalidated\"} 1\n"
    ));
    assert!(metrics.contains(
        "adx_coordinator_node_available_cpu_millis{shard_id=\"0\",node_id=\"node\"} 0\n"
    ));

    assert_eq!(
        snapshot.environments["held"].result.as_ref().unwrap().state,
    ...
    let manager = Adxlet::new(
        "node".into(),
        backend.clone(),
        Arc::new(LocalChecks),
        Arc::new(LocalChecks),
        Arc::new(
            CoordinatorStateSink::new(channel(address, "node").await, Duration::from_secs(2))
                .unwrap()
...
async fn node_restart_before_expiry_requires_new_session_at_registered_endpoint() {
    ...
    let manager = |boot: &str, channel| {
        Arc::new(Adxlet::new(
            ...
            Arc::new(LocalChecks),
            Arc::new(
                CoordinatorStateSink::new(channel, Duration::from_secs(2))
                    .unwrap()
                    .with_session(boot.into()),
            ),
        ))
    };
    let serve_node = |listener: TcpListener, manager: Arc<Adxlet>, boot: &str| {
    ...
    let running = frontend
        .create_environment(create("held"))
        .await
        .unwrap()
        .into_inner()
        .record
    ...
    report.reconciling = false;
    report.accepting_allocations = true;
    node_coordinator.register_node(report).await.unwrap();
    assert_eq!(session.snapshot().await.unwrap().routes().unwrap().len(), 1);
...
#[tokio::test]
#[ignore = "requires isolated Redis and mTLS certificates; run control-rpc suite"]
async fn coordinator_restart_bounds_re_registration_grace() {
    use adx_core::{Assignment, EnvironmentState};
    for mode in ["timer", "late-register", "timely-register"] {
        ...
        session
            .register_session(
                adx_coordinator::Node::try_from(report.clone()).unwrap(),
        ...
        if mode == "timer" {
            assert_eq!(
                ...
                1,
                "unregistered node must expire after Coordinator recovery grace"
            );
            assert_eq!(rpc.expire_nodes().await.unwrap(), 0);
        }
...
#[tokio::test]
#[ignore = "requires isolated Redis and mTLS certificates; run control-rpc suite"]
async fn published_routes_drive_real_gateway_streams_and_reconnect_to_new_coordinator() {
    use adx_coordinator::{
        auth::{AuthRpc, Credential},
        routes::RoutePublisher,
    };
    use adx_core::{Assignment, EnvironmentState};
    use adxlet::{routes::UdsRoutes, Routes as _};
    ...
    session
        ...
        .unwrap();
    let mut forbidden =
        pb::route_service_client::RouteServiceClient::new(channel(ma, "apiserver").await);
    assert_eq!(
        forbidden
    ...
    let ingress = Ingress::new(
        Arc::new(IngressRouteResolver::new(store.clone()).stream_only()),
        DataPlaneL4Connector::new(H2PoolConfig::default()),
        IngressAuthenticator::with_verifier(connection),
        target_port,
        target_port,
        "127.0.0.1:1",
        vec![],
    ...
    let mut stream = ingress
        ...
        .await
        .unwrap();
    let payload = b"through-coordinator-routes";
    ...
    drop(cached);
    // Local retirement protects stale Ingress cache before Coordinator publishes the deletion.
...
async fn snapshot_rpc_enforces_component_tenant_and_deferred_deletion() {
    ...
    let mut ingress =
        pb::snapshot_service_client::SnapshotServiceClient::new(channel(address, "ingress").await);
    assert_eq!(
        ingress.get_snapshot(get).await.unwrap_err().code(),
        tonic::Code::PermissionDenied
    );
    assert_eq!(
        client
...
async fn tenant_key_rpc_requires_frontend_admin_and_revokes_verification() {
    let redis = common::Redis::new().await;
    let session = redis.store().await.begin(1).await.unwrap();
    let auth = adx_coordinator::auth::AuthRpc::new(session, peers());
    let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
    ...
    let mut ingress =
        pb::credential_service_client::CredentialServiceClient::new(channel(addr, "ingress").await);
    assert_eq!(
        ingress
            .create_tenant_key(request.clone())
            .await
            .unwrap_err()
            .code(),
        tonic::Code::PermissionDenied
    ...
    let mut verify =
        pb::auth_service_client::AuthServiceClient::new(channel(addr, "ingress").await);
    let identity = verify
        .verify_api_key(pb::VerifyApiKeyRequest {
...
#[async_trait::async_trait]
impl adxlet::checkpoint::CheckpointCooperation for LocalChecks {
...
async fn snapshot_gc_waits_for_node_ack_and_recovers_from_lost_ack() {
    ...
    use adxlet::checkpoint::{CheckpointStore, LocalCheckpointStore};
    let redis = common::Redis::new().await;
    let session = redis.store().await.begin(1).await.unwrap();
    ...
    let mut servers = Servers(vec![tokio::spawn(async move {
        Server::builder()
            ...
            .add_service(pb::coordinator_service_server::CoordinatorServiceServer::new(serving))
            .serve_with_incoming(TcpListenerStream::new(ml))
            .await
            .unwrap();
    })]);
    ...
    session
        ...
        .await
        .unwrap();
    assert_eq!(coordinator.collect_snapshots().await.unwrap(), 0);
    assert!(path.exists());
    ...
    assert!(coordinator.collect_snapshots().await.is_err());
    assert_eq!(
        session.get_snapshot(&snapshot.id).await.unwrap().state,
        SnapshotState::Deleting
    );
    ...
    manager
        ...
        .await
        .unwrap();
    // Node completed physical deletion but Coordinator did not receive/commit the ack.
    ...
    assert_eq!(coordinator.collect_snapshots().await.unwrap(), 1);
    assert_eq!(
        session.get_snapshot(&snapshot.id).await.unwrap().state,
    ...
    let mut api = pb::coordinator_service_client::CoordinatorServiceClient::new(
        channel(ma, "apiserver").await,
    );
    let source = api
        .create_environment(create("snapshot-source"))
        .await
        .unwrap()
        .into_inner()
        .record
    ...
    assert_eq!(
        saved.environment.unwrap().record.unwrap().state,
    ...
    assert_eq!(
        api.create_environment(denied).await.unwrap_err().code(),
        tonic::Code::InvalidArgument
    );
    let created = frontend
        .create_local_environment(pb::LocalEnvironmentCreateRequest {
    ...
    assert!(session
        ...
        .contains(&Reference::Restore {
            environment_id: "snapshot-clone".into()
        }));
    assert_eq!(coordinator.collect_snapshots().await.unwrap(), 0);
    assert!(session
        .get_snapshot(&reusable.id)
    ...
    assert_eq!(
        api.create_environment(request)
            .await
            .unwrap()
            .into_inner()
            .record
...
#[tokio::test]
#[ignore = "requires isolated Redis and generated mTLS certificates; run control-rpc suite"]
async fn coordinator_restart_releases_snapshot_pins_for_lost_memory_queue_only() {
    ...
    first
        .acquire_snapshot(
            ...
            "tenant",
            Reference::Restore {
                environment_id: "memory-queue-only".into(),
            },
        )
        .await
        .unwrap();
    ...
    let _coordinator = CoordinatorRpc::new(
        second.clone(),
        Placement::Pack,
        peers(),
        client_tls("coordinator"),
...
impl RuntimeDriver for RecoveryBackend {
    ...
    async fn restore_from(
        ...
        assert_eq!(origin.environment_id, spec.id);
...
async fn shared_checkpoint_moves_to_new_node_and_old_node_cleans_without_deleting_artifact() {
    use adx_core::{Assignment, EnvironmentState, RestorePoint};
    use adxlet::checkpoint::{CheckpointStore, ObjectCheckpointStore, RemoteGcConfig};
    let redis = common::Redis::new().await;
    let session = redis.store().await.begin(2).await.unwrap();
    let test_peers = || {
        Peers::new([
            (file("coordinator.der"), Principal::Coordinator),
            (file("node.der"), Principal::Node("source".into())),
            (file("apiserver.der"), Principal::Node("target".into())),
        ])
    };
    let rpc = CoordinatorRpc::with_heartbeat_timeout(
        session.clone(),
    ...
    let manager = Arc::new(
        Adxlet::new(
            ...
            Arc::new(LocalChecks),
            Arc::new(
                CoordinatorStateSink::new(channel(ma, "apiserver").await, Duration::from_secs(3))
                    .unwrap()
    ...
    let mut source =
        pb::coordinator_service_client::CoordinatorServiceClient::new(channel(ma, "node").await);
    let mut target = pb::coordinator_service_client::CoordinatorServiceClient::new(
        channel(ma, "apiserver").await,
    );
    ...
    target.register_node(target_report.clone()).await.unwrap();
    assert_eq!(rpc.expire_nodes().await.unwrap(), 1);
    assert_eq!(rpc.recover_environments().await.unwrap(), 1);
    let restored = session.get("held").await.unwrap();
    let result = restored.result.clone().unwrap();
    assert_eq!(result.state, EnvironmentState::Running);
    ...
    assert_eq!(rpc.recover_environments().await.unwrap(), 0);
    assert_eq!(backend.0.started.load(Ordering::SeqCst), 1);
    let mut duplicate = result.clone();
    duplicate.state = EnvironmentState::Paused;
    ...
    let mut coordinator_node =
        pb::node_service_client::NodeServiceClient::new(channel(na, "coordinator").await);
    coordinator_node
        .recover_environment(pb::RecoverEnvironmentRequest {
    ...
    let returned = Adxlet::new(
        "source".into(),
        old_backend.clone(),
        Arc::new(LocalChecks),
        Arc::new(LocalChecks),
        Arc::new(
            CoordinatorStateSink::new(channel(ma, "node").await, Duration::from_secs(3))
                .unwrap()
