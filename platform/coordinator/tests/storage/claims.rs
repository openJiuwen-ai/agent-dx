use super::*;
use adx_coordinator::storage::{ClaimOutcome, LocalClaim, NodeSession};

async fn claimant(s: &adx_coordinator::storage::Session, id: &str) -> LocalClaim {
    s.register_session(
        node(id),
        "127.0.0.1:9000".into(),
        "127.0.0.1:9001".into(),
...
#[tokio::test]
#[ignore = "requires dedicated real Redis; build/ci/run.py storage"]
async fn claims_fence_old_coordinator_node_sessions_and_closed_admission() {
    let rig = common::Redis::new().await;
    let db = rig.store().await;
    let s = db.begin(1).await.unwrap();
    let a = claimant(&s, "a").await;
...
async fn local_claim_and_scheduler_reservation_cannot_create_two_owners() {
    ...
    assert_eq!(s.snapshot().await.unwrap().environments.len(), 1);
}
...
async fn snapshot_claim_requires_existing_restore_reference() {
    use adx_coordinator::snapshots::{Reference, Snapshot};
    let rig = common::Redis::new().await;
    let db = rig.store().await;
    let s = db.begin(1).await.unwrap();
    let a = claimant(&s, "a").await;
