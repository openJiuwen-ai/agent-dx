//! Deployment-owned runtime root and bootstrap, persisted with each Environment.
...
impl RuntimeProfile {
    pub fn validate(&self) -> Result<()> {
        ...
        {
            return Err(Error::Invalid("runtime profile requires one matching local/EROFS or OCI image source, an absolute target and an entrypoint beneath it".into()));
        }
        Ok(())
    }
}
