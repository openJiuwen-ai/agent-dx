use adx_core::{Assignment, CapsuleRecord, CapsuleSpec, CapsuleState, Resources, Runtime};
...
fn environment_roundtrip_preserves_oci_source() {
    let environment: adx_core::environment::EnvironmentSpec =
    ...
    let wire: control::EnvironmentSpec = environment.clone().into();
    assert_eq!(
        adx_core::environment::EnvironmentSpec::try_from(wire).unwrap(),
        environment
    );
}
...
fn record_roundtrip_preserves_identity_state_and_ownership() {
    let spec = CapsuleSpec {
        environment: Some(serde_json::from_value(serde_json::json!({
    ...
    assert_eq!(
        serde_json::from_slice::<CapsuleRecord>(&persisted).unwrap(),
        r
    );
    let wire = control::CapsuleRecord::try_from(r.clone()).unwrap();
    assert_eq!(CapsuleRecord::try_from(wire.clone()).unwrap(), r);
    let mut invalid = wire.clone();
    invalid.state = 0;
    assert!(CapsuleRecord::try_from(invalid).is_err());
    let mut invalid = wire;
    invalid.assignment.as_mut().unwrap().capsule_id = "other".into();
    assert!(CapsuleRecord::try_from(invalid).is_err());
}
