...
use adx_node_manager::{Durability, NodeManager, Readiness, Routes, RuntimeDriver, StateSink};
...
impl Readiness for Dependencies {
    async fn wait_ready(&self, _record: &CapsuleRecord) -> Result<()> {
        self.event("ready")
    }
}
#[async_trait]
impl Routes for Dependencies {
    async fn activate(&self, _record: &CapsuleRecord) -> Result<()> {
        self.event("activate")
    }
    async fn retire(&self, _record: &CapsuleRecord) -> Result<()> {
        self.event("retire")
    }
}
#[async_trait]
...
async fn create_publishes_only_after_runtime_readiness_and_route_binding() {
    ...
    let node = node(&deps);
    let capsule = node.capsule(spec("a"), assignment("a")).unwrap();
    let created = capsule.create().await.unwrap();
    assert_eq!(created.record.state, CapsuleState::Running);
    assert_eq!(created.durability, Durability::Published);
    assert_eq!(
        *deps.events.lock().unwrap(),
        ["start", "ready", "activate", "commit:Running"]
    );
    capsule.create().await.unwrap();
    assert_eq!(deps.events.lock().unwrap().len(), 4);
    assert_eq!(node.used(), resources());
    capsule.delete().await.unwrap();
    ...
    assert_eq!(capsule.create().await.unwrap_err(), Error::Conflict);
}
...
async fn network_policy_replacement_is_serialized_and_published_idempotently() {
    ...
    let capsule = node.capsule(value, assignment("network")).unwrap();
    let created = capsule.create().await.unwrap();
...
async fn failed_readiness_is_cleaned_before_resources_are_reused() {
    ...
    *deps.fail_at.lock().unwrap() = Some("ready".into());
    let node = node(&deps);
    let capsule = node.capsule(spec("a"), assignment("a")).unwrap();
    assert!(capsule.create().await.is_err());
    assert_eq!(
        *deps.events.lock().unwrap(),
...
async fn failed_runtime_cleanup_keeps_reservation_and_can_be_retried() {
    ...
    *deps.fail_at.lock().unwrap() = Some("remove".into());
    assert!(capsule.delete().await.is_err());
    assert_eq!(node.used(), resources());
    assert!(capsule.sync().await.unwrap().record.resources_held);
...
#[tokio::test]
async fn node_without_any_valid_capacity_sample_rejects_new_capsules() {
    let deps = Dependencies::new(false);
    let node = NodeManager::new(
        "n1".into(),
        deps.clone(),
    ...
    let capsule = node.capsule(spec("a"), assignment("a")).unwrap();
    assert_eq!(capsule.create().await.unwrap_err(), Error::NoCapacity);
...
async fn timed_out_start_requires_confirmed_cleanup_before_releasing_capacity() {
    ...
    let node = node(&deps)
        ...
        .unwrap();
    let capsule = node.capsule(spec("a"), assignment("a")).unwrap();
    let task = tokio::spawn(async move { capsule.create().await });
...
async fn deletion_commit_retry_does_not_repeat_cleanup() {
    ...
    *deps.fail_at.lock().unwrap() = Some("commit:Deleted".into());
    ...
    assert_eq!(node.used(), Resources::default());
    *deps.fail_at.lock().unwrap() = None;
    let result = capsule.delete().await.unwrap();
    assert_eq!(result.record.state, CapsuleState::Deleted);
...
async fn journaled_result_is_explicitly_unpublished_and_can_be_synced() {
    ...
    *deps.durability.lock().unwrap() = Durability::Published;
    let synced = capsule.sync().await.unwrap();
...
async fn failed_commit_does_not_claim_success_or_repeat_runtime_operation() {
    ...
    *deps.fail_at.lock().unwrap() = None;
    assert_eq!(
        capsule.create().await.unwrap().record.state,
...
async fn delete_waits_for_active_operation_even_if_original_caller_disconnects() {
    ...
    let creating = tokio::spawn({
        ...
        async move { capsule.create().await }
    });
    deps.start_entered.acquire().await.unwrap().forget();
    creating.abort();
    let deleting = tokio::spawn({
        let capsule = capsule.clone();
        async move { capsule.delete().await }
    });
...
async fn wrong_node_or_generation_cannot_acquire_existing_controller() {
    let deps = Dependencies::new(false);
    let node = node(&deps);
    node.capsule(spec("a"), assignment("a")).unwrap();
    let mut other = assignment("a");
    other.generation = 2;
    assert!(node.capsule(spec("a"), other).is_err());
    let mut other = assignment("b");
    other.node_id = "n2".into();
    assert!(node.capsule(spec("b"), other).is_err());
}
...
async fn local_device_admission_is_atomic_and_failed_cleanup_keeps_cards_reserved() {
    ...
    let first = node.capsule(a.clone(), assigned.clone()).unwrap();
    first.create().await.unwrap();
    a.id = "b".into();
    assigned.capsule_id = "b".into();
    let second = node.capsule(a, assigned).unwrap();
...
async fn stale_device_inventory_closes_accelerator_admission() {
    ...
    let handle = node.capsule(a, assigned).unwrap();
...
async fn explicit_drain_requires_published_deletion_and_closes_admission() {
    let deps = Dependencies::new(false);
    let node = node(&deps);
    node.capsule(spec("a"), assignment("a"))
        .unwrap()
        .create()
        .await
        .unwrap();
    *deps.fail_at.lock().unwrap() = Some("commit:Deleted".into());
    ...
    assert!(node.capsule(spec("b"), assignment("b")).is_err());
    *deps.fail_at.lock().unwrap() = None;
...
async fn metrics_follow_reservations_and_capacity_shrink() {
    ...
    let node = node(&deps);
    let capsule = node
        .capsule(spec("metrics"), assignment("metrics"))
        .unwrap();
    capsule.create().await.unwrap();
...
#[tokio::test]
async fn accepted_capsule_operations_keep_request_context_after_caller_cancellation() {
    let _provider = adx_observability::trace::init("queue-test").unwrap();
    let deps = Dependencies::new(true);
    let node = node(&deps);
    let capsule = node.capsule(spec("trace"), assignment("trace")).unwrap();
    let first = capsule.clone();
    ...
    let delete = tokio::spawn(
        ...
        .run(async move { capsule.delete().await }),
    );
    deps.start_release.add_permits(1);
...
async fn local_reservations_share_one_hold_and_transfer_to_one_controller() {
    ...
    let x = manager.capsule(spec("a"), assignment("a")).unwrap();
    let y = manager.capsule(spec("a"), assignment("a")).unwrap();
...
async fn local_gpu_npu_tokens_transfer_and_stale_release_cannot_free_cards() {
    ...
    let capsule = manager.capsule(requested.clone(), assigned).unwrap();
    capsule.create().await.unwrap();
