...
#[async_trait]
impl adx_node_manager::checkpoint::CheckpointStore for UnavailablePublication {
    ...
    ) -> Result<adx_node_manager::checkpoint::MaterializedCheckpoint> {
...
async fn reconciliation_preserves_ready_and_deleting_reusable_snapshots() {
    ...
    use adx_node_manager::checkpoint::CheckpointStore;
...
#[async_trait]
impl adx_node_manager::checkpoint::SnapshotCatalog for Backend {
...
async fn recovery_retries_publication_without_repeating_backend_restore() {
    use adx_node_manager::checkpoint::{CheckpointStore, ObjectCheckpointStore};
