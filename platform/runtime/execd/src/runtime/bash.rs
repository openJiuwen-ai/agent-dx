...
//! Semantics match `environment.py`: persistent bash attaches to a PTY, submit writes `cmd; echo SENTINEL$?`, and poll completes after reading the sentinel.
...
const EXECD_PROMPT: &str = "__EXECD_PROMPT__ ";
...
pub fn bash_init(kw: &BTreeMap<String, Value>) -> Value {
    ...
    cmd.env("PS1", EXECD_PROMPT);
    cmd.env("PS2", EXECD_PROMPT);
    ...
    // and make the PTY framing ambiguous.  EXECD sessions are an API transport,
...
pub fn bash_submit(kw: &BTreeMap<String, Value>) -> Value {
    ...
    // Command plus sentinel carrying the exit code. __EXECD_DONE_<n>__ distinguishes echoed `$?` from the actual numeric code.
    let line = format!("{command}\necho __EXECD_DONE_$?__\n");
    match session
        .writer
...
pub fn bash_poll(kw: &BTreeMap<String, Value>) -> Value {
    ...
    // Find __EXECD_DONE_<digits>__: skip echoed `echo __EXECD_DONE_$?__` entries where `$?` is non-numeric.
    let marker = "__EXECD_DONE_";
    let mut from = 0;
...
mod tests {
    ...
    fn deterministic_prompt_preserves_output_without_trailing_newline() {
        ...
        let stdout = field(&result, "stdout")
            ...
            .expect("stdout");
        assert!(
            stdout.contains("stateful__EXECD_PROMPT__ echo __EXECD_DONE_$?__"),
