...
impl BoundedBuffer {
    fn append(&mut self, bytes: &[u8]) {
        ...
        let global_limit = env_usize(
            "EXECD_COMMAND_REGISTRY_MAX_BYTES",
...
fn output_limit(name: &str) -> usize {
    std::env::var(name)
        ...
        .unwrap_or_else(|| {
            env_usize(
                "EXECD_COMMAND_OUTPUT_LIMIT_BYTES",
...
fn make_room(records: &mut HashMap<String, CommandRecord>) -> bool {
    ...
    let max = env_usize(
        "EXECD_COMMAND_REGISTRY_MAX_RECORDS",
        DEFAULT_REGISTRY_MAX_RECORDS,
    );
    let max_bytes = env_usize(
        "EXECD_COMMAND_REGISTRY_MAX_BYTES",
        DEFAULT_REGISTRY_MAX_BYTES,
    );
...
fn request_fingerprint(kw: &BTreeMap<String, Value>) -> String {
    ...
    hasher.update(b"adx-command-request-v1\0");
...
pub fn cmd_start(kw: &BTreeMap<String, Value>) -> Value {
    ...
    records.insert(
        ...
        CommandRecord {
            ...
            stdin_closed: false,
            stdin: None,
            stdout: empty_reader("EXECD_COMMAND_STDOUT_LIMIT_BYTES"),
            stderr: empty_reader("EXECD_COMMAND_STDERR_LIMIT_BYTES"),
            exit: Arc::clone(&exit),
        },
    );
    ...
    let stdout = spawn_reader(child.stdout.take(), "EXECD_COMMAND_STDOUT_LIMIT_BYTES");
    let stderr = spawn_reader(child.stderr.take(), "EXECD_COMMAND_STDERR_LIMIT_BYTES");
...
pub fn cmd_wait(kw: &BTreeMap<String, Value>) -> Value {
    ...
    if wait_exit(&exit, timeout).is_none() {
        return map_value(vec![
            ...
            ("error_message", Value::from("command wait timed out")),
            ("error", nil()),
        ]);
    }
    cmd_get(kw)
...
mod tests {
    ...
    fn legacy_kill_and_spawn_failure_terminate_polling() {
        ...
        let failed = cmd_start(&BTreeMap::from([
            ...
            (
                "cwd".to_string(),
                Value::from("/definitely-not-a-real-adx-command-directory"),
            ),
        ]));
    ...
    #[test]
    fn wait_deadline_returns_running_result_without_stopping_command() {
        let id = format!("test-wait-deadline-{}", now_ms());
        cmd_start(&args(&id, "sleep 5"));
        let lookup = BTreeMap::from([
    ...
    #[test]
    fn killing_missing_command_is_a_successful_noop() {
        let lookup = BTreeMap::from([(
            "command_id".to_string(),
