//! Environment runtime boot, HTTP operations and checkpoint listener recovery.
use adx_core::runtime::RuntimeIdentity;
use std::sync::Arc;
use std::time::{Duration, SystemTime, UNIX_EPOCH};
use tokio::sync::watch;

macro_rules! execd_info {
    ($($arg:tt)*) => {
        $crate::runtime::log_info(format_args!($($arg)*))
    };
}

macro_rules! execd_warn {
    ($($arg:tt)*) => {
        $crate::runtime::log_warn(format_args!($($arg)*))
    };
}

macro_rules! execd_error {
    ($($arg:tt)*) => {
...
pub async fn run() -> Result<(), Box<dyn std::error::Error>> {
    let tunnel = if std::env::var_os("EXECD_TUNNEL_WS_PORT").is_some() {
        let ws = port("EXECD_TUNNEL_WS_PORT", 8765)?;
        Some((
            ws,
            port("EXECD_TUNNEL_HTTP_PORT", ws.checked_add(1).unwrap_or(0))?,
        ))
    } else {
        None
    };
    boot(
        port("EXECD_HTTP_PORT", 50090)?,
        std::env::var("EXECD_HTTP_TOKEN").ok(),
        tunnel,
    )
    .await
}
fn identity_from(
    ...
    let identity = RuntimeIdentity {
        environment_id: required(keys[0])?,
...
impl control::CheckpointHooks for RuntimeHooks {
    ...
    fn restore_context(
        ...
        if let Some(value) = environment.get("EXECD_HTTP_PORT") {
        ...
        if let Some(token) = environment.get("EXECD_HTTP_TOKEN") {
...
async fn boot(
    ...
    let checkpoint = match std::env::var_os("ADX_EXECD_CONTROL_SOCKET_PATH")
        .filter(|v| !v.is_empty())
    {
        Some(directory) => {
            if identity.is_none() {
...
> {
    let listener = httpserver::bind(port).await.map_err(|err| {
        let message = format!("failed to bind EXECD HTTP port {port}: {err}");
        execd_error!("[execd-http] readiness failed: {message}");
    ...
    execd_info!("[execd-http] readiness ready address={address}");
    Ok((ready_rx, control))
}
...
> {
    let bound = tunnel::BoundTunnelServers::bind(ws_port, http_port)
        .await
        .map_err(|message| {
            execd_error!("[execd-tunnel] readiness failed: {message}");
            message
        })?;
    let (ready_tx, ready_rx) = watch::channel(RuntimeReadyState::Starting);
    ...
    execd_info!("[execd-tunnel] readiness ready ws=0.0.0.0:{ws_port} http=127.0.0.1:{http_port}");
    Ok((ready_rx, control))
}
...
async fn wait_for_runtime_ready(
    ...
    loop {
        ...
        if ready.changed().await.is_err() {
            return Err(
                "EXECD service readiness channel closed before startup completed".to_string(),
            );
        }
    }
}
