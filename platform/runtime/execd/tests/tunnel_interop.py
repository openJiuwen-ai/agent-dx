...
"""Real TCP -> Rust EXECD -> sandbox-sdk -> HTTP/1 framing checks."""
...
def wait_for_runtime(port):
    ...
    raise RuntimeError("adx-execd HTTP tunnel port did not become ready")
...
def main():
    ...
    proc = subprocess.Popen(
        [os.environ.get("EXECD_RUNTIME", "adx-execd")],
        env={
            **os.environ,
            "EXECD_TUNNEL_ONLY": "1",
            "EXECD_TUNNEL_WS_PORT": str(ws_port),
            "EXECD_TUNNEL_HTTP_PORT": str(http_port),
        },
    )
    tunnel_client = None
    passed = []
    try:
        ...
        payload = b'{"message":"chunked-through-execd"}'
        chunked = (
