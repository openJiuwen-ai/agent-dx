...
`rrt-runtime` serves Capsule operations over HTTP: `/healthz`, `/invoke`, command watches, uploads and downloads. Configure `RRT_HTTP_PORT` (default 50090) and optional `RRT_HTTP_TOKEN`. Reverse tunnels use HTTP/WebSocket and are enabled with `RRT_TUNNEL_WS_PORT` and `RRT_TUNNEL_HTTP_PORT`.

The public Edge → Node Proxy → RRT operations are defined in the
...
Node Manager supplies `ADX_CAPSULE_ID`, `ADX_RUNTIME_ID` and `ADX_OWNERSHIP_GENERATION` through sandboxd. Runtime cooperation uses the same HTTP listener: identity-aware status, checkpoint preparation and confirmed-unstarted abort. See the [HTTP contract](../../api/http/runtime-control.md).
