"""Live adx-sandbox -> rrt direct-connection e2e.

Requires a deployed sandbox cluster. Wires the adx-sandbox to the frontend
(``ADX_SERVER_ADDRESS``). RRT direct invoke uses ``/direct`` on that endpoint,
...
test: together they cover adx-sandbox -> rrt direct verification (contract + real cluster).
